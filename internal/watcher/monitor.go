...
// [ADDED] Fungsi pintar untuk Switch Dev Mode secara Real-time tanpa Restart!
func ToggleDevMode(isDev bool, notifier SocketNotifier) {
	...
	for _, dir := range dirs {
		...
		for _, entry := range entries {
			...
			if IsDevMode {
				// UNHIDE: Temukan folder yang disembunyikan (.raw_) dan kembalikan namanya!
				if strings.HasPrefix(folderName, ".raw_") {
					originalName := strings.TrimPrefix(folderName, ".raw_")
					oldPath := filepath.Join(dir, folderName)
					newPath := filepath.Join(dir, originalName)
					os.Rename(oldPath, newPath)
					log.Printf("[🛠️ Dev Mode] Restored/Unhidden raw folder: %s", originalName)
				}
			} else {
				// ENCRYPT: Temukan folder mentah yang belum diamankan dan amankan!
				if !strings.HasPrefix(folderName, ".") && folderName != "libs" {
					log.Printf("[🔒 Security] Dev Mode Disabled. Auto-securing folder: %s", folderName)
