//#######################################################################
// File NAME : internal/socket/handler.go
//#######################################################################
...
import (
	...
	"flowork-engine/internal/watcher" // Memanggil status Dev Mode global
...
type SocketHandler struct {
	...
	engineDir    string                   // [KODE BARU] Absolute path engine directory
}

// =========================================================================
// Struct kustom untuk mencegat dan menggandakan output Log
// =========================================================================
...
func (w *socketLogWriter) Write(p []byte) (n int, err error) {
	msg := string(p)
	// Tembakkan log ke antarmuka GUI Vue secara real-time
	w.server.Of("/gui-socket", nil).Emit("engine:log", map[string]interface{}{
...
// [KODE BARU] Custom MultiWriter Anti-Gagal untuk Windows EXE
// Menggantikan io.MultiWriter bawaan Go agar log tetap mengalir ke Web UI
// meskipun os.Stdout crash/mati karena tidak ada terminal.
type safeLogDispatcher struct {
	writers []io.Writer
}

func (s *safeLogDispatcher) Write(p []byte) (n int, err error) {
	for _, w := range s.writers {
		w.Write(p) // Eksekusi semua writer dan abaikan jika ada yang error
	}
	return len(p), nil
}
// =========================================================================

func NewSocketHandler(nm *runner.NodeManager, engineDir string) *SocketHandler {
	...
	handler := &SocketHandler{
		...
		engineDir:   engineDir,                      // [KODE BARU] Lock ke absolute path
	}

	// [PERBAIKAN BUG] Gunakan safeLogDispatcher, JANGAN gunakan io.MultiWriter
	log.SetOutput(&safeLogDispatcher{
		writers: []io.Writer{os.Stdout, &socketLogWriter{server: ioServer}},
	})
...
// BroadcastSwarmTask dispatches a swarm automation command to ALL connected peers.
// This enables distributed multi-device browser automation from the AI Builder.
func (s *SocketHandler) BroadcastSwarmTask(payload map[string]interface{}) {
	log.Printf("\n[Engine] 🐝 SWARM DISPATCH: Broadcasting task to all connected peers...\n")
	s.io.Of("/gui-socket", nil).Emit("engine:swarm_task", payload)
}
...
func (s *SocketHandler) setupEvents() {
	...
	namespace.On("connection", func(clients ...any) {
		...
		client.On("engine:toggle_dev_mode", func(args ...any) {
			if len(args) > 0 {
				if payload, ok := args[0].(map[string]interface{}); ok {
					...
					// Trigger auto-sweep untuk mengembalikan/menyembunyikan file tanpa harus restart!
					watcher.ToggleDevMode(isDev, s)
				}
			}
		})

		// =========================================================================
		// Handler to EXIT/SHUTDOWN ENGINE from GUI
		// =========================================================================
		...
		// =========================================================================
		// Handler to RESTART ENGINE (Fix Mental CMD)
		// =========================================================================
		client.On("engine:restart", func(args ...any) {
			...
			// Tambahkan SysProcAttr agar proses baru TIDAK MUNCUL CMD-nya!
			if runtime.GOOS == "windows" {
				cmd.SysProcAttr = &syscall.SysProcAttr{HideWindow: true}
			}

			cmd.Start() // Start new instance
		...
		client.On("engine:get_installed_ids", func(args ...any) {
			...
			appEntries, _ := os.ReadDir(filepath.Join(s.engineDir, "apps"))
			...
			nodeEntries, _ := os.ReadDir(filepath.Join(s.engineDir, "nodes"))
		...
		client.On("engine:uninstall_module", func(args ...any) {
			...
			targetDir := filepath.Join(s.engineDir, "apps") // [KODE BARU] Absolute path
			ext := ".flow"
			if moduleType == "node" {
				targetDir = filepath.Join(s.engineDir, "nodes") // [KODE BARU] Absolute path
				ext = ".nflow"
			}
			...
			if err != nil && !os.IsNotExist(err) {
				...
				// [KODE BARU] Sinkronisasi otomatis ke GUI setelah file dihapus
				if moduleType == "node" {
					updatedNodes := s.nodeManager.ScanNodes()
					namespace.Emit("engine:nodes_list", map[string]interface{}{
						"data": updatedNodes,
					})
				}
			}
		...
		// =========================================================================
		// Handler for Direct Download to Portable Folder
		// =========================================================================
		client.On("engine:install_module", func(args ...any) {
			...
			go func() {
				...
				// [KODE BARU] Git LFS Bypass Router
				// Merubah link raw.githubusercontent.com menjadi github.com/raw
				// agar GitHub otomatis me-redirect kita ke file binary asli (media.githubusercontent.com)
				if strings.HasPrefix(downloadURL, "https://raw.githubusercontent.com/") {
					dl := strings.Replace(downloadURL, "raw.githubusercontent.com", "github.com", 1)
					dl = strings.Replace(dl, "/refs/heads/", "/raw/", 1)
					downloadURL = dl
					log.Printf("[Engine] 🔀 GitHub LFS URL Bypassed to: %s", downloadURL)
				}
				...
				// [KODE BARU] Push update struktur Nodes otomatis ke Web GUI.
				// Ini menyelesaikan masalah "Properties Node kosong setelah diinstal".
				if moduleType == "node" {
					// Beri jeda 500ms agar OS Windows menyelesaikan file lock flushing hard disk
					time.Sleep(500 * time.Millisecond)
					updatedNodes := s.nodeManager.ScanNodes()
					namespace.Emit("engine:nodes_list", map[string]interface{}{
						"data": updatedNodes,
					})
					log.Printf("[Engine] 🔄 Pushed updated physical nodes structure to GUI Sidebar.\n")
				}
			}()
		})
		// =========================================================================
		...
		client.On("engine:get_apps", func(args ...any) {
			...
			entries, err := os.ReadDir(filepath.Join(s.engineDir, "apps")) // [KODE BARU] Absolute path
			if err == nil {
				for _, entry := range entries {
					if !entry.IsDir() && strings.HasSuffix(entry.Name(), ".flow") {
						appID := strings.TrimSuffix(entry.Name(), ".flow")
						flowPath := filepath.Join(s.engineDir, "apps", entry.Name()) // [KODE BARU]

						// 100% In-Memory Reading
						manifestBytes, err := packer.ExtractFileInMemory(flowPath, "manifest.json")
						if err == nil {
							...
							log.Printf("[Engine] ⚠️ Failed to extract manifest.json from %s: %v\n", entry.Name(), err)
						}

					} else if entry.IsDir() && watcher.IsDevMode && !strings.HasPrefix(entry.Name(), ".") && entry.Name() != "libs" {
						// DEV MODE: Read manifest.json directly from raw folder!
						appID := entry.Name()
						manifestPath := filepath.Join(s.engineDir, "apps", entry.Name(), "manifest.json") // [KODE BARU]
		...
		client.On("engine:execute_task", func(args ...any) {
			...
			go func() {
				// Deteksi Bypass untuk Dev Mode
				isRawDevApp := false
				appRawPath := filepath.Join(s.engineDir, "apps", taskName) // [KODE BARU] Absolute path
				...
				if isRawDevApp {
					...
					appFlowPath := filepath.Join(s.engineDir, "apps", taskName+".flow") // [KODE BARU]
...
// [PERBAIKAN KODE] Helper untuk Engine Router
func (s *SocketHandler) GetSandboxPath(appName string) (string, bool) {
	s.sandboxMutex.Lock()
	defer s.sandboxMutex.Unlock()

	sandbox, exists := s.sandboxes[appName]
	if exists {
		return sandbox.Path, true
	}
