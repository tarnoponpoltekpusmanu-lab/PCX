...
func openAppWindow(url string) {
    ...
    if _, err := os.Stat(electronExePath); err == nil {
        ...
        log.Printf("[Engine] ⚠️ Executable Webview (%s) tidak ditemukan di %s.\n", electronExeName, baseDir)

        // [KODE BARU] Menggunakan variabel global IsDevModeGlobal
        if IsDevModeGlobal {
            log.Printf("[Engine] ⚠️ Mencoba menjalankan via 'npm start' (Dev Mode)...\n")
            ...
            err := cmd.Start()
            if err != nil {
                log.Printf("[Engine] ❌ Gagal menjalankan Webview Electron: %v\n", err)
                openBrowser(url)
            }
        } else {
            log.Printf("[Engine] 🌐 Fallback membuka Browser Standar karena Webview/GUI tidak tersedia.\n")
            openBrowser(url)
        }
    }
}
...
func main() {
    ...
    app.Post("/api/upload", func(c *fiber.Ctx) error {
        ...
        // [DIBERIKAN KOMENTAR] targetDir := "apps"
        targetDir := filepath.Join(EngineDir, "apps") // [KODE BARU] Gunakan absolute path
        ext := ".flow"
        if fileType == "node" {
            // [DIBERIKAN KOMENTAR] targetDir = "nodes"
            targetDir = filepath.Join(EngineDir, "nodes") // [KODE BARU] Gunakan absolute path
            ext = ".nflow"
        }
    ...
    app.Get("/local-apps/:appName/*", func(c *fiber.Ctx) error {
        ...
        isRawDevApp := false
        // [DIBERIKAN KOMENTAR] rawAppPath := filepath.Join("apps", appName)
        rawAppPath := filepath.Join(EngineDir, "apps", appName) // [KODE BARU] Gunakan absolute path
    ...
    nodeManager := runner.NewNodeManager(filepath.Join(EngineDir, "nodes")) // [KODE BARU]
