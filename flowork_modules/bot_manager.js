...
module.exports = {
    ...
    init: function(ipcMain, FloworkState, childProcess, pathModule, appModule, baseDir, fs, session) {
        ...
        ipcMain.handle('app:add-device', async (event, { id, url, partition, uaMode, useProxy, proxyAddress, useGhostCursor, useBandwidthSaver, useSleepCycle, workMins, sleepMins }) => {
            ...
            // [AI BROWSER BRIDGE] Notify hooks that a new device was created
            if (FloworkState.methods.onDeviceCreated) {
                FloworkState.methods.onDeviceCreated(id, view);
            }
        ...
        ipcMain.handle('app:go-back', (event, id) => {
            if (id === '__main_flowork__' && FloworkState.mainFloworkView) {
                if (FloworkState.mainFloworkView.webContents.canGoBack()) {
                    FloworkState.mainFloworkView.webContents.goBack();
                }
                return { success: true };
            }

            if (FloworkState.appTabs && FloworkState.appTabs[id] && FloworkState.appTabs[id].view) {
                if (FloworkState.appTabs[id].view.webContents.canGoBack()) {
                    FloworkState.appTabs[id].view.webContents.goBack();
                }
                return { success: true };
            }
