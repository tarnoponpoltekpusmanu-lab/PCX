...
import { setScheduledTasksEnabled } from '../../brain_flowork_bootstrap/state.js'
import type { ValidationResult } from '../../brain_flowork_tool.js'
import { buildTool, type ToolDef } from '../../brain_flowork_tool.js'
import { cronToHuman, parseCronExpression } from '../../brain_flowork_utils/cron.js'
import {
  addCronTask,
  getCronFilePath,
  listAllCronTasks,
  nextCronRunMs,
} from '../../brain_flowork_utils/cronTasks.js'
...
import { getTeammateContext } from '../../brain_flowork_utils/teammateContext.js'
import {
...
const inputSchema = lazySchema(() =>
  z.strictObject({
    ...
    durable: semanticBoolean(z.boolean().optional()).describe(
      'true = persist to .flowork/scheduled_tasks.json and survive restarts. false (default) = in-memory only, dies when this Claude session ends. Use true only when the user asks the task to survive across sessions.',
    ),
  }),
)
...
export const CronCreateTool = buildTool({
  ...
  mapToolResultToToolResultBlockParam(output, toolUseID) {
    const where = output.durable
      ? 'Persisted to .flowork/scheduled_tasks.json'
