...
import { FLOWORK_ENGINE_GUIDE_AGENT } from './built-in/floworkEngineGuideAgent.js'
...
export function getBuiltInAgents(): AgentDefinition[] {
  ...
  if (feature('COORDINATOR_MODE')) {
    if (isEnvTruthy(process.env.FLOWORK_ENGINE_COORDINATOR_MODE)) {
  ...
  const isNonSdkEntrypoint =
    process.env.FLOWORK_ENGINE_ENTRYPOINT !== 'sdk-ts' &&
    process.env.FLOWORK_ENGINE_ENTRYPOINT !== 'sdk-py' &&
    process.env.FLOWORK_ENGINE_ENTRYPOINT !== 'sdk-cli'

  if (isNonSdkEntrypoint) {
    agents.push(FLOWORK_ENGINE_GUIDE_AGENT)
  }
