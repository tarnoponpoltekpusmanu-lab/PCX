...
export const PREAPPROVED_HOSTS = new Set([
  // FloworkOS
  'platform.flowork.com',
  'code.flowork.com',
  'modelcontextprotocol.io',
  'github.com/floworkoss',
  'agentskills.io',
...
// for the handful of path-scoped entries (e.g., "github.com/floworkoss").
...
export function isPreapprovedHost(hostname: string, pathname: string): boolean {
  ...
  if (prefixes) {
    for (const p of prefixes) {
      // Enforce path segment boundaries: "/floworkoss" must not match
      // "/floworkoss-evil/malware". Only exact match or a "/" after the
      // prefix is allowed.
