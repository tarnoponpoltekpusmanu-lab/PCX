...
import { isCurrentDirectoryBareGitRepo } from '../../brain_flowork_utils/git.js'
import type { PermissionRule } from '../../brain_flowork_utils/permissions/PermissionRule.js'
...
import { containsVulnerableUncPath } from '../../brain_flowork_utils/shell/readOnlyCommandValidation.js'
...
export async function powershellToolHasPermission(
  ...
  // `Write-Output $env:FLOWORKOS_API_KEY`, pushes allow to decisions[], and
  ...
  for (const { text: subCmd, element, statement } of subCommands) {
    ...
    if (
      ...
      // `Write-Output $env:FLOWORKOS_API_KEY`. Apply the same argLeaksValue
    ...
    // gate, `Set-Location ./.flowork; Set-Content ./settings.json '...'` would
    ...
    // .flowork/settings.json (a Claude config file the path validator didn't check).
