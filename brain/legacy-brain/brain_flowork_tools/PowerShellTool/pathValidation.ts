...
import type { PermissionRule } from '../../brain_flowork_types/permissions.js'
import { getCwd } from '../../brain_flowork_utils/cwd.js'
import {
  getFsImplementation,
  safeResolvePath,
} from '../../brain_flowork_utils/fsOperations.js'
import { containsPathTraversal, getDirectoryForPath } from '../../brain_flowork_utils/path.js'
import {
...
} from '../../brain_flowork_utils/powershell/parser.js'
import {
...
function isPathAllowed(
  ...
  // This MUST come before checkPathSafetyForAutoEdit since .flowork is a dangerous directory
  // and internal editable paths live under ~/.flowork/ — matching the ordering in
  ...
  for (let i = 0; i < args.length; i++) {
    ...
    if (isPowerShellParameter(arg, argElementType)) {
      ...
      if (matchesParam(paramLower, config.pathParams)) {
        ...
        // SECURITY: A Variable elementType (e.g., $env:FLOWORKOS_API_KEY) in any
...
function checkPathConstraintsForStatement(
  ...
  //   Set-Location ./.flowork; Set-Content ./settings.json '...'
  // Validator sees ./settings.json → /project/settings.json (not a config file).
  // Runtime writes /project/.flowork/settings.json (Flowork's permission config).
  //
  // ALTERNATIVE APPROACH (rejected): simulate cwd through the statement chain
  // — after `Set-Location ./.flowork`, validate subsequent statements with
  // cwd='./.flowork'. This would be more permissive but requires careful
  // handling of:
      ...
      if (!allowed) {
        ...
        const message =
          ...
          decisionReason?.type === 'safetyCheck'
            ...
            : `${canonical} targeting '${resolvedPath}' was blocked. For security, Flowork AI Engine may only access files in the allowed working directories for this session: ${dirListStr}.`
