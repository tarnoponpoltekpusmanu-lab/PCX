 ...
 * viewer (they're on Flowork's machine). We upload to /api/oauth/file_upload —
...
} from '../../brain_flowork_bridge/bridgeConfig.js'
 ...
 *
 * Subprocess hosts (cowork) pass FLOWORKOS_BASE_URL alongside
 * FLOWORK_ENGINE_OAUTH_TOKEN — prefer that since getOauthConfig() only
 * returns staging when USE_STAGING_OAUTH is set, which such hosts don't
 * set. Without this a staging token hits api.floworkos.com → 401 → silent
