...
import { IMAGE_EXTENSION_REGEX } from '../../brain_flowork_utils/imagePaste.js'
...
export async function resolveAttachments(
  ...
  // of the guard inside uploadBriefAttachment — FLOWORK.md: "helpers defined
  ...
  if (feature('BRIDGE_MODE')) {
    ...
    // REPL does, at main.tsx init). FLOWORK_ENGINE_BRIEF_UPLOAD lets a host that
    // runs the CLI as a subprocess opt in — e.g. the cowork desktop bridge,
    // which already passes FLOWORK_ENGINE_OAUTH_TOKEN for auth.
    const shouldUpload =
      uploadCtx.replBridgeEnabled ||
      isEnvTruthy(process.env.FLOWORK_ENGINE_BRIEF_UPLOAD)
