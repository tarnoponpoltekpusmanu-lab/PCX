...
import { isReplBridgeActive } from '../../brain_flowork_bootstrap/state.js'
import { getReplBridgeHandle } from '../../brain_flowork_bridge/replBridgeHandle.js'
import type { Tool, ToolUseContext } from '../../brain_flowork_tool.js'
import { buildTool, type ToolDef } from '../../brain_flowork_tool.js'
import { findTeammateTaskByAgentId } from '../../brain_flowork_tasks_dir/InProcessTeammateTask/InProcessTeammateTask.js'
import {
  isLocalAgentTask,
  queuePendingMessage,
} from '../../brain_flowork_tasks_dir/LocalAgentTask/LocalAgentTask.js'
import { isMainSessionTask } from '../../brain_flowork_tasks_dir/LocalMainSessionTask.js'
import { toAgentId } from '../../brain_flowork_types/ids.js'
import { generateRequestId } from '../../brain_flowork_utils/agentId.js'
...
import { gracefulShutdown } from '../../brain_flowork_utils/gracefulShutdown.js'
import { lazySchema } from '../../brain_flowork_utils/lazySchema.js'
import { parseAddress } from '../../brain_flowork_utils/peerAddress.js'
...
import { TEAM_LEAD_NAME } from '../../brain_flowork_utils/swarm/constants.js'
import { readTeamFileAsync } from '../../brain_flowork_utils/swarm/teamHelpers.js'
import {
  getAgentId,
  getAgentName,
  getTeammateColor,
...
async function handleBroadcast(
  ...
  if (!teamName) {
    throw new Error(
      'Not in a team context. Create a team with Teammate spawnTeam first, or set FLOWORK_ENGINE_TEAM_NAME.',
    )
  }
  ...
  if (!senderName) {
    throw new Error(
      'Cannot broadcast: sender name is required. Set FLOWORK_ENGINE_AGENT_NAME.',
    )
  }
    ...
    async checkPermissions(input, _context) {
      if (feature('UDS_INBOX') && parseAddress(input.to).scheme === 'bridge') {
        return {
          ...
          message: `Send a message to Remote Control session ${input.to}? It arrives as a user prompt on the receiving Claude (possibly another machine) via FloworkOS's servers.`,
      ...
      if (feature('UDS_INBOX') && typeof input.message === 'string') {
        ...
        if (addr.scheme === 'uds') {
          ...
          const { sendToUdsSocket } =
            require('../../brain_flowork_utils/udsClient.js') as typeof import('../../utils/udsClient.js')
          /* eslint-enable @typescript-eslint/no-require-imports */
          try {
