...
import type { Redirect, SimpleCommand } from '../../brain_flowork_utils/bash/ast.js'
import {
  extractOutputRedirections,
  splitCommand_DEPRECATED,
} from '../../brain_flowork_utils/bash/commands.js'
import { tryParseShellCommand } from '../../brain_flowork_utils/bash/shellQuote.js'
import { getDirectoryForPath } from '../../brain_flowork_utils/path.js'
import { allWorkingDirectories } from '../../brain_flowork_utils/permissions/filesystem.js'
import type { PermissionResult } from '../../brain_flowork_utils/permissions/PermissionResult.js'
import { createReadRuleSuggestion } from '../../brain_flowork_utils/permissions/PermissionUpdate.js'
import type { PermissionUpdate } from '../../brain_flowork_utils/permissions/PermissionUpdateSchema.js'
import {
  expandTilde,
  ...
  validatePath,
} from '../../brain_flowork_utils/permissions/pathValidation.js'
 ...
 *
 *   rm -- -/../.flowork/settings.local.json
 *
 * Here `-/../.flowork/settings.local.json` starts with `-` so the naive filter
...
function validateCommandPaths(
  ...
  if (validator && !validator(args)) {
    return {
      behavior: 'ask',
      message: `${command} with flags requires manual approval to ensure path safety. For security, Flowork AI Engine cannot automatically validate ${command} commands that use flags, as some flags like --target-directory=PATH can bypass path validation.`,
      decisionReason: {
        type: 'other',
  ...
  // Example attack: cd .flowork/ && mv test.txt settings.json
  // This would bypass the check for .flowork/settings.json because paths are resolved
  ...
  // effective CWD through the command chain (e.g., after "cd .flowork/", subsequent
  // commands would be validated with CWD=".flowork/"). This would be more permissive
  ...
  if (compoundCommandHasCd && operationType !== 'read') {
    return {
      behavior: 'ask',
      message: `Commands that change directories and perform write operations require explicit approval to ensure paths are evaluated correctly. For security, Flowork AI Engine cannot automatically determine the final working directory when 'cd' is used in compound commands.`,
      decisionReason: {
        type: 'other',
        reason:
    ...
    if (!allowed) {
      ...
      const message =
        ...
        decisionReason?.type === 'safetyCheck'
          ...
          : `${command} in '${resolvedPath}' was blocked. For security, Flowork AI Engine may only ${ACTION_VERBS[command]} the allowed working directories for this session: ${dirListStr}.`
...
function validateOutputRedirections(
  ...
  // Example attack: cd .flowork/ && echo "malicious" > settings.json
  ...
  if (compoundCommandHasCd && redirections.length > 0) {
    return {
      behavior: 'ask',
      message: `Commands that change directories and write via output redirection require explicit approval to ensure paths are evaluated correctly. For security, Flowork AI Engine cannot automatically determine the final working directory when 'cd' is used in compound commands.`,
      decisionReason: {
        type: 'other',
        reason:
      ...
      const message =
        ...
        decisionReason?.type === 'safetyCheck'
          ...
          : decisionReason?.type === 'rule'
            ...
            : `Output redirection to '${resolvedPath}' was blocked. For security, Flowork AI Engine may only write to files in the allowed working directories for this session: ${dirListStr}.`
