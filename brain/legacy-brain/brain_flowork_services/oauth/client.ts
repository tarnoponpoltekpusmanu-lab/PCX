// OAuth client for handling authentication flows with Flowork AI services
...
import type { AccountInfo } from '../../brain_flowork_utils/config.js'
...
export async function refreshOAuthToken(
  ...
  try {
    ...
    // FLOWORK_ENGINE_OAUTH_REFRESH_TOKEN re-login path: installOAuthTokens runs
...
export async function populateOAuthAccountInfoIfNeeded(): Promise<boolean> {
  ...
  const envAccountUuid = process.env.FLOWORK_ENGINE_ACCOUNT_UUID
  const envUserEmail = process.env.FLOWORK_ENGINE_USER_EMAIL
  const envOrganizationUuid = process.env.FLOWORK_ENGINE_ORGANIZATION_UUID
