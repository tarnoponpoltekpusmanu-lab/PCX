...
import { getOriginalCwd } from '../../brain_flowork_bootstrap/state.js'
import { isBuiltinPluginId } from '../../brain_flowork_plugins/builtinPlugins.js'
import type { LoadedPlugin, PluginManifest } from '../../brain_flowork_types/plugin.js'
...
} from '../../brain_flowork_utils/plugins/cacheUtils.js'
import {
  findReverseDependents,
  formatReverseDependentsSuffix,
} from '../../brain_flowork_utils/plugins/dependencyResolver.js'
import {
  loadInstalledPluginsFromDisk,
  loadInstalledPluginsV2,
  removePluginInstallation,
  updateInstallationPathOnDisk,
} from '../../brain_flowork_utils/plugins/installedPluginsManager.js'
import {
  getMarketplace,
  getPluginById,
  loadKnownMarketplacesConfig,
} from '../../brain_flowork_utils/plugins/marketplaceManager.js'
import { deletePluginDataDir } from '../../brain_flowork_utils/plugins/pluginDirectories.js'
import {
  parsePluginIdentifier,
  scopeToSettingSource,
} from '../../brain_flowork_utils/plugins/pluginIdentifier.js'
import {
  formatResolutionError,
  installResolvedPlugin,
} from '../../brain_flowork_utils/plugins/pluginInstallationHelpers.js'
import {
  cachePlugin,
  ...
  loadAllPlugins,
  loadPluginManifest,
} from '../../brain_flowork_utils/plugins/pluginLoader.js'
import { deletePluginOptions } from '../../brain_flowork_utils/plugins/pluginOptionsStorage.js'
...
import { calculatePluginVersion } from '../../brain_flowork_utils/plugins/pluginVersioning.js'
import type {
  PluginMarketplaceEntry,
  PluginScope,
} from '../../brain_flowork_utils/plugins/schemas.js'
import {
...
/**
 * Is this plugin enabled (value === true) in .flowork/settings.json?
 *
...
export async function uninstallPluginOp(
  ...
  if (!scopeInstallation) {
    ...
    if (actualScope !== scope && installations && installations.length > 0) {
      // Project scope is special: .flowork/settings.json is shared with the team.
      // Point users at the local-override escape hatch instead of --scope project.
      if (actualScope === 'project') {
        return {
          success: false,
          message: `Plugin "${plugin}" is enabled at project scope (.flowork/settings.json, shared with your team). To disable just for you: claude plugin disable ${plugin} --scope local`,
        }
      }
      return {
        success: false,
...
export async function setPluginEnabledOp(
  ...
  // without touching the shared .flowork/settings.json).
...
async function performPluginUpdate({
  ...
  if (typeof entry.source !== 'string') {
    ...
    const manifestPath = join(sourcePath, '.flowork-plugin', 'plugin.json')
    try {
