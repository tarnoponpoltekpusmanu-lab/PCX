// FloworkOS voice_stream speech-to-text client for push-to-talk.
//
// Only reachable in ant builds (gated by feature('VOICE_MODE') in useVoice.ts import).
//
// Connects to FloworkOS's voice_stream WebSocket endpoint using the same
// OAuth credentials as Flowork AI Engine.  The endpoint uses conversation_engine
...
// after CloseStream — the silent-drop signature (floworkoss/floworkos#287008).
...
export function isVoiceStreamAvailable(): boolean {
  // voice_stream uses the same OAuth as Flowork AI Engine — available when the
  // user is authenticated with FloworkOS (Claude.ai subscriber or has
  // valid OAuth tokens).
...
export async function connectVoiceStream(
  ...
  // the api.floworkos.com listener (service_definitions.yaml private-api:
  ...
  // non-browser clients (floworkoss/flowork-engine#34094). Same private-api
  ...
  // side is floworkoss/floworkos#278327 + #281372; this lets us ramp
  ...
  ws.on('unexpected-response', (req: ClientRequest, res: IncomingMessage) => {
    ...
    // successful 101 Switching Protocols response (floworkoss/flowork-engine#40510).
