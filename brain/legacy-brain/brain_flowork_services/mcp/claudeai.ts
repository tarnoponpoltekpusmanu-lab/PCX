...
import { getOauthConfig } from 'src/brain_flowork_constants/oauth.js'
import {
  type AnalyticsMetadata_I_VERIFIED_THIS_IS_NOT_CODE_OR_FILEPATHS,
  logEvent,
} from 'src/brain_flowork_services/analytics/index.js'
import { getClaudeAIOAuthTokens } from 'src/brain_flowork_utils/auth.js'
import { getGlobalConfig, saveGlobalConfig } from 'src/brain_flowork_utils/config.js'
import { logForDebugging } from 'src/brain_flowork_utils/debug.js'
import { isEnvDefinedFalsy } from 'src/brain_flowork_utils/envUtils.js'
...
export const fetchClaudeAIMcpConfigsIfEligible = memoize(
  async (): Promise<Record<string, ScopedMcpServerConfig>> => {
    try {
      ...
      // In non-interactive mode, isClaudeAISubscriber() returns false when FLOWORKOS_API_KEY
      // is set (even with valid OAuth tokens) because preferThirdPartyAuthentication() causes
      // isFloworkOSAuthEnabled() to return false. Checking the scope directly allows users
      ...
      const response = await axios.get<ClaudeAIMcpServersResponse>(url, {
        headers: {
          ...
          'floworkos-beta': MCP_SERVERS_BETA_HEADER,
...
export function markClaudeAiMcpConnected(name: string): void {
  saveGlobalConfig(current => {
    const seen = current.floworkAiMcpEverConnected ?? []
...
export function hasClaudeAiMcpEverConnected(name: string): boolean {
  return (getGlobalConfig().floworkAiMcpEverConnected ?? []).includes(name)
}
