...
import xss from 'xss'
import { MCP_CLIENT_METADATA_URL } from '../../brain_flowork_constants/oauth.js'
import { openBrowser } from '../../brain_flowork_utils/browser.js'
import { getClaudeConfigHomeDir } from '../../brain_flowork_utils/envUtils.js'
import { errorMessage, getErrnoCode } from '../../brain_flowork_utils/errors.js'
import * as lockfile from '../../brain_flowork_utils/lockfile.js'
import { logMCPDebug } from '../../brain_flowork_utils/log.js'
import { getPlatform } from '../../brain_flowork_utils/platform.js'
import { getSecureStorage } from '../../brain_flowork_utils/secureStorage/index.js'
import { clearKeychainCache } from '../../brain_flowork_utils/secureStorage/macOsKeychainHelpers.js'
import type { SecureStorageData } from '../../brain_flowork_utils/secureStorage/types.js'
 ...
 * Used by Flowork AIAuthProvider for metadata discovery and token refresh.
 ...
 *
 * Per RFC 7009, public clients (like Flowork AI Engine) should authenticate by including
...
export async function performMCPOAuthFlow(
  ...
  //
  // Servers with `oauth.xaa` but FLOWORK_ENGINE_ENABLE_XAA unset hard-fail with
  // actionable copy rather than silently degrade to consent.
  if (serverConfig.oauth?.xaa) {
    if (!isXaaEnabled()) {
      throw new Error(
        `XAA is not enabled (set FLOWORK_ENGINE_ENABLE_XAA=1). Remove 'oauth.xaa' from server '${serverName}' to use the standard consent flow.`,
      )
    }
      ...
      server = createServer((req, res) => {
        ...
        if (parsedUrl.pathname === '/callback') {
          ...
          if (code) {
            res.writeHead(200, { 'Content-Type': 'text/html' })
            res.end(
              `<h1>Authentication Successful</h1><p>You can close this window. Return to Flowork AI Engine.</p>`,
            )
            cleanup()
            resolveOnce(code)
          }
 ...
 * to the PKCE flow. See github.com/floworkoss/flowork-engine/issues/28258.
 */
...
export class ClaudeAuthProvider implements OAuthClientProvider {
  ...
  get clientMetadata(): OAuthClientMetadata {
    const metadata: OAuthClientMetadata = {
      client_name: `Flowork AI Engine (${this.serverName})`,
   ...
   * but this is the shape FLOWORK.md flags under "Token/auth caching across
  ...
  async refreshAuthorization(
    ...
    const lockfilePath = join(floworkDir, `mcp-refresh-${sanitizedKey}.lock`)
