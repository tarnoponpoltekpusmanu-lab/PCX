...
import { logAntError } from '../../brain_flowork_utils/debug.js'
...
import { logError, logMCPDebug, logMCPError } from '../../brain_flowork_utils/log.js'
...
export async function getMcpHeadersFromHelper(
  ...
  try {
    ...
    const execResult = await execFileNoThrowWithCwd(config.headersHelper, [], {
      ...
      // (git credential-helper style). See deshaw/floworkos-issues#28.
      env: {
        ...process.env,
        FLOWORK_ENGINE_MCP_SERVER_NAME: serverName,
        FLOWORK_ENGINE_MCP_SERVER_URL: config.url,
      },
    })
