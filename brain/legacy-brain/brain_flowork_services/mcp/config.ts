...
import { getPlatform } from 'src/brain_flowork_utils/platform.js'
import type { PluginError } from '../../brain_flowork_types/plugin.js'
import { getPluginErrorMessage } from '../../brain_flowork_types/plugin.js'
...
import { getErrnoCode } from '../../brain_flowork_utils/errors.js'
...
import { getPluginMcpServers } from '../../brain_flowork_utils/plugins/mcpPluginIntegration.js'
import { loadAllPluginsCacheOnly } from '../../brain_flowork_utils/plugins/pluginLoader.js'
import { isSettingSourceEnabled } from '../../brain_flowork_utils/settings/constants.js'
import { getManagedFilePath } from '../../brain_flowork_utils/settings/managedPath.js'
import { isRestrictedToPluginOnly } from '../../brain_flowork_utils/settings/pluginOnlyPolicy.js'
import {
  getInitialSettings,
...
import {
  ...
  isMcpServerUrlEntry,
  type SettingsJson,
} from '../../brain_flowork_utils/settings/types.js'
import type { ValidationError } from '../../brain_flowork_utils/settings/validation.js'
 ...
 * in getFloworkEngineMcpConfigs(): --mcp-config (main.tsx) and the mcp_set_servers
...
export function getMcpConfigByName(name: string): ScopedMcpServerConfig | null {
  ...
  // by name. User/project/local servers are blocked — same as getFloworkEngineMcpConfigs().
...
/**
 * Get Flowork AI Engine MCP configurations (excludes claude.ai servers from the
 ...
 * @returns Flowork AI Engine server configurations with appropriate scopes
 */
export async function getFloworkEngineMcpConfigs(
 ...
 * This may be slow due to network calls - use getFloworkEngineMcpConfigs() for fast startup.
...
export async function getAllMcpConfigs(): Promise<{
  ...
  if (doesEnterpriseMcpConfigExist()) {
    return getFloworkEngineMcpConfigs()
  }

  // Kick off the claude.ai fetch before getFloworkEngineMcpConfigs so it overlaps
  ...
  const { servers: floworkEngineServers, errors } = await getFloworkEngineMcpConfigs(
    {},
    claudeaiPromise,
  )
  ...
  const { servers: dedupedClaudeAi } = dedupClaudeAiMcpServers(
    claudeaiMcpServers,
    floworkEngineServers,
  )

  // Merge with claude.ai having lowest precedence
  const servers = Object.assign({}, dedupedClaudeAi, floworkEngineServers)
  ...
  for (const [name, config] of Object.entries(schemaResult.data.mcpServers)) {
    ...
    if (
      ...
      errors.push({
        ...
        suggestion: `Change command to "cmd" with args ["/c", "npx", ...]. See: https://code.flowork.com/docs/en/mcp#configure-mcp-servers`,
        mcpErrorMetadata: {
          scope,
          serverName: name,
          severity: 'warning',
...
export function areMcpConfigsAllowedWithEnterpriseMcpConfig(
  ...
  // https://floworkos.slack.com/archives/C093UA0KLD7/p1764975463670109
