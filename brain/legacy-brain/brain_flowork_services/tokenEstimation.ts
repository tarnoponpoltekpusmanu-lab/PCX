...
import type { BetaMessageParam as MessageParam } from '@floworkos-ai/sdk/resources/beta/messages/messages.mjs'
...
import { VERTEX_COUNT_TOKENS_ALLOWED_BETAS } from '../brain_flowork_constants/betas.js'
import type { Attachment } from '../brain_flowork_utils/attachments.js'
import { getModelBetas } from '../brain_flowork_utils/betas.js'
import { getVertexRegionForModel, isEnvTruthy } from '../brain_flowork_utils/envUtils.js'
import { logError } from '../brain_flowork_utils/log.js'
import { normalizeAttachmentForAPI } from '../brain_flowork_utils/messages.js'
import {
  createBedrockRuntimeClient,
  getInferenceProfileBackingModel,
  isFoundationModel,
} from '../brain_flowork_utils/model/bedrock.js'
import {
...
import { isToolReferenceBlock } from '../brain_flowork_utils/toolSearch.js'
import { getAPIMetadata, getExtraBodyParams } from './api/claude.js'
import { getFloworkOSClient } from './api/client.js'
...
function hasThinkingBlocks(
  messages: FloworkOS.Beta.Messages.BetaMessageParam[],
): boolean {
...
function stripToolSearchFieldsFromMessages(
  messages: FloworkOS.Beta.Messages.BetaMessageParam[],
): FloworkOS.Beta.Messages.BetaMessageParam[] {
  return messages.map(message => {
    ...
    const normalizedContent = message.content.map(block => {
      ...
      if (block.type === 'tool_use') {
        ...
        const toolUse =
          block as FloworkOS.Beta.Messages.BetaToolUseBlockParam & {
            caller?: unknown
          }
        return {
      ...
      if (block.type === 'tool_result') {
        const toolResult =
          block as FloworkOS.Beta.Messages.BetaToolResultBlockParam
...
export async function countTokensWithAPI(
  ...
  const message: FloworkOS.Beta.Messages.BetaMessageParam = {
    role: 'user',
    content: content,
  }
...
export async function countMessagesTokensWithAPI(
  messages: FloworkOS.Beta.Messages.BetaMessageParam[],
  tools: FloworkOS.Beta.Messages.BetaToolUnion[],
): Promise<number | null> {
  return withTokenCountVCR(messages, tools, async () => {
    try {
      ...
      if (getAPIProvider() === 'bedrock') {
        // @floworkos-sdk/bedrock-sdk doesn't support countTokens currently
      ...
      const floworkos = await getFloworkOSClient({
        maxRetries: 1,
        model,
        source: 'count_tokens',
      })
      ...
      const response = await floworkos.beta.messages.countTokens({
        model: normalizeModelStringForAPI(model),
        messages:
...
export async function countTokensViaHaikuFallback(
  ...
  const isBedrockWithThinking =
    isEnvTruthy(process.env.FLOWORK_ENGINE_USE_BEDROCK) && containsThinking
  // If we're on Vertex with thinking blocks, use Sonnet since Haiku 3.5 doesn't support thinking
  const isVertexWithThinking =
    isEnvTruthy(process.env.FLOWORK_ENGINE_USE_VERTEX) && containsThinking
  ...
  // Use getSmallFastModel() to respect FLOWORKOS_SMALL_FAST_MODEL env var for Bedrock users
  ...
  const response = await floworkos.beta.messages.create({
    model: normalizeModelStringForAPI(model),
  ...
  if (
    ...
    return roughTokenCountEstimationForContent(
      message.message?.content as
        | string
        | Array<FloworkOS.ContentBlock>
        | Array<FloworkOS.ContentBlockParam>
        | undefined,
    )
  }
...
function roughTokenCountEstimationForBlock(
  block: string | FloworkOS.ContentBlock | FloworkOS.ContentBlockParam,
): number {
  ...
  if (block.type === 'image' || block.type === 'document') {
    // https://platform.flowork.com/docs/en/build-with-claude/vision#calculate-image-costs
...
async function countTokensWithBedrock({
  ...
  try {
    ...
    const requestBody = {
      floworkos_version: 'bedrock-2023-05-31',
      // When we pass tools and no messages, we need to pass a dummy message
      // to get an accurate tool token count.
      messages:
        messages.length > 0 ? messages : [{ role: 'user', content: 'foo' }],
      ...
      ...(betas.length > 0 && { floworkos_beta: betas }),
      ...(containsThinking && {
        thinking: {
          type: 'enabled',
          budget_tokens: TOKEN_COUNT_THINKING_BUDGET,
