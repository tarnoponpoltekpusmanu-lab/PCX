...
import { getOauthConfig, OAUTH_BETA_HEADER } from '../../brain_flowork_constants/oauth.js'
import {
...
import { classifyAxiosError, getErrnoCode } from '../../brain_flowork_utils/errors.js'
import { settingsChangeDetector } from '../../brain_flowork_utils/settings/changeDetector.js'
import {
  type SettingsJson,
  SettingsSchema,
...
function getRemoteSettingsAuthHeaders(): {
  ...
  // Wrap in try-catch because getFloworkOSApiKeyWithSource throws in CI/test environments
  try {
    const { key: apiKey } = getFloworkOSApiKeyWithSource({
  ...
  // Fall back to OAuth tokens (for Flowork.ai users)
