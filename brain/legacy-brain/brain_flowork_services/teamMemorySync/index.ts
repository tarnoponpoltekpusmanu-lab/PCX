 ...
 *
 * API contract (floworkos/floworkos#250711 + #283027):
...
import {
  ...
  getOauthConfig,
  OAUTH_BETA_HEADER,
} from '../../brain_flowork_constants/oauth.js'
import {
  getTeamMemPath,
  PathTraversalError,
  validateTeamMemKey,
} from '../../brain_flowork_memdir/teamMemPaths.js'
...
import { classifyAxiosError } from '../../brain_flowork_utils/errors.js'
import { getGithubRepo } from '../../brain_flowork_utils/git.js'
import {
  getAPIProvider,
  isFirstPartyFloworkOSBaseUrl,
} from '../../brain_flowork_utils/model/providers.js'
...
// Per-entry size cap — server default from floworkos/floworkos#293258.
...
export type SyncState = {
   ...
   * (floworkos/floworkos#293258 adds error_code + extra_details.max_entries).
 ...
 * Format matches the server's entryChecksums values (floworkos/floworkos#283027)
...
function isUsingOAuth(): boolean {
  if (getAPIProvider() !== 'firstParty' || !isFirstPartyFloworkOSBaseUrl()) {
    return false
  }
 ...
 * Requires floworkos/floworkos#283027 deployed; on failure the caller fails the
...
async function fetchTeamMemoryHashes(
  ...
  try {
    ...
    // Requires floworkos/floworkos#283027. If entryChecksums is missing,
...
async function uploadTeamMemory(
  ...
  try {
    ...
    // Parse structured 413 (floworkos/floworkos#293258). The server's
...
async function readLocalTeamMemory(maxEntries: number | null): Promise<{
  ...
  // 413's extra_details.max_entries — floworkos/floworkos#293258).  The
...
export async function pullTeamMemory(
  ...
  // Requires floworkos/floworkos#283027 — if the response lacks entryChecksums
...
export async function pushTeamMemory(
  ...
  for (
    ...
    if (!result.conflict) {
      ...
      // max_entries (floworkos/floworkos#293258), cache it so the next push
    ...
    if (!probe.success || !probe.entryChecksums) {
      // Requires floworkos/floworkos#283027. A transient probe failure here is
