...
import type { MessageParam } from '@floworkos-ai/sdk/resources/index.mjs'
...
import { getSmallFastModel } from '../brain_flowork_utils/model/model.js'
...
function extractRawUtilization(headers: globalThis.Headers): RawUtilization {
  ...
  for (const [key, abbrev] of [
    ...
    const util = headers.get(
      `floworkos-ratelimit-unified-${abbrev}-utilization`,
    )
    const reset = headers.get(`floworkos-ratelimit-unified-${abbrev}-reset`)
...
async function makeTestQuery() {
  ...
  return floworkos.beta.messages
    .create({
      model,
      max_tokens: 1,
      messages,
  ...
  for (const [claimAbbrev, rateLimitType] of Object.entries(
    ...
    if (surpassedThreshold !== null) {
      const utilizationHeader = headers.get(
        `floworkos-ratelimit-unified-${claimAbbrev}-utilization`,
      )
      const resetHeader = headers.get(
        `floworkos-ratelimit-unified-${claimAbbrev}-reset`,
      )
...
function computeNewLimitsFromHeaders(
  ...
  const status =
    (headers.get('floworkos-ratelimit-unified-status') as QuotaStatus) ||
    'allowed'
  const resetsAtHeader = headers.get('floworkos-ratelimit-unified-reset')
  const resetsAt = resetsAtHeader ? Number(resetsAtHeader) : undefined
  const unifiedRateLimitFallbackAvailable =
    headers.get('floworkos-ratelimit-unified-fallback') === 'available'
...
function cacheExtraUsageDisabledReason(headers: globalThis.Headers): void {
  ...
  const reason =
    headers.get('floworkos-ratelimit-unified-overage-disabled-reason') ?? null
