 ...
 *
 * This module provides functionality to download and upload files to FloworkOS Public Files API.
 * Used by the Flowork AI Engine agent to download file attachments at session startup.
 *
 * API Reference: https://docs.floworkos.com/en/api/files-content
 */
...
// API base URL - uses FLOWORKOS_BASE_URL set by env-manager for the appropriate environment
// Falls back to public API for standalone usage
function getDefaultApiBaseUrl(): string {
  return (
    process.env.FLOWORKOS_BASE_URL ||
    process.env.FLOWORK_ENGINE_API_BASE_URL ||
...
export type FilesApiConfig = {
  ...
  oauthToken: string
  /** Base URL for the API (default: https://api.floworkos.com) */
  baseUrl?: string
...
/**
 * Downloads a single file from the FloworkOS Public Files API
 *
...
export async function downloadFile(
  ...
  const headers = {
    ...
    'floworkos-beta': FILES_API_BETA_HEADER,
  }
