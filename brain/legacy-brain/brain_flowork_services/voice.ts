...
import { isEnvTruthy, isRunningOnHomespace } from '../brain_flowork_utils/envUtils.js'
...
export async function checkRecordingAvailability(): Promise<RecordingAvailability> {
  ...
  if (isRunningOnHomespace() || isEnvTruthy(process.env.FLOWORK_ENGINE_REMOTE)) {
    return {
      available: false,
      reason:
        'Voice mode requires microphone access, but no audio device is available in this environment.\n\nTo use voice mode, run Flowork AI Engine locally instead.',
    }
  }
  ...
  const wslNoAudioReason =
    'Voice mode could not access an audio device in WSL.\n\nWSL2 with WSLg (Windows 11) provides audio via PulseAudio — if you are on Windows 10 or WSL1, run Flowork AI Engine in native Windows instead.'
