...
import { getSessionCreatedTeams } from '../../brain_flowork_bootstrap/state.js'
...
import { getTasksDir, notifyTasksUpdated } from '../brain_flowork_tasks.js'
...
export async function cleanupTeamDirectories(teamName: string): Promise<void> {
  ...
  // Clean up team directory (~/.flowork/teams/{team-name}/)
  const teamDir = getTeamDir(teamName)
  try {
  ...
  // Clean up tasks directory (~/.flowork/tasks/{taskListId}/)
