import { isRemoteManagedSettingsEligible } from '../brain_flowork_services/remoteManagedSettings/syncCache.js'
...
/**
 * `claude ssh` remote: FLOWORKOS_UNIX_SOCKET routes auth through a -R forwarded
 * socket to a local proxy, and the launcher sets a handful of placeholder auth
 * env vars that the remote's ~/.flowork settings.env MUST NOT clobber (see
 * isFloworkOSAuthEnabled). Strip them from any settings-sourced env object.
 */
function withoutSSHTunnelVars(
  env: Record<string, string> | undefined,
): Record<string, string> {
  if (!env || !process.env.FLOWORKOS_UNIX_SOCKET) return env || {}
  const {
    FLOWORKOS_UNIX_SOCKET: _1,
    FLOWORKOS_BASE_URL: _2,
    FLOWORKOS_API_KEY: _3,
    FLOWORKOS_AUTH_TOKEN: _4,
    FLOWORK_ENGINE_OAUTH_TOKEN: _5,
    ...rest
  } = env
  return rest
}
 ...
 * FLOWORK_ENGINE_PROVIDER_MANAGED_BY_HOST in spawn env), strip
 * provider-selection / model-default vars from settings-sourced env so a
 * user's ~/.flowork/settings.json can't redirect requests away from the
...
function withoutHostManagedProviderVars(
  ...
  if (!env) return {}
  if (!isEnvTruthy(process.env.FLOWORK_ENGINE_PROVIDER_MANAGED_BY_HOST)) {
    return env
  }
 ...
 *
 * - userSettings (~/.flowork/settings.json): controlled by the user, not project-specific
 ...
 * traffic (e.g., FLOWORKOS_BASE_URL) to an attacker-controlled server.
 */
 ...
 * FLOWORKOS_BASE_URL take effect during first-run/onboarding.
 *
 * For trusted sources (user settings, managed settings, CLI flags), ALL env vars
 * are applied — including ones like FLOWORKOS_BASE_URL that would be dangerous
...
export function applySafeConfigEnvironmentVariables(): void {
  ...
  // Global config (~/.flowork.json) is user-controlled. In CCD mode,
  ...
  // doesn't get clobbered by ~/.flowork/settings.json env (gh#217). policy/flag
  ...
  // flagSettings env applied. Eligibility reads FLOWORK_ENGINE_USE_BEDROCK,
  // FLOWORKOS_BASE_URL — both settable via settings.env.
  ...
  // when FLOWORK_ENGINE_PROVIDER_MANAGED_BY_HOST is set.
 ...
 * FLOWORK_ENGINE_PROVIDER_MANAGED_BY_HOST is set — see filterSettingsEnv) and
