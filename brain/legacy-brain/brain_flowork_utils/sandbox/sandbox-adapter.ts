/**
 * Adapter layer that wraps @floworkos-ai/sandbox-runtime with Flowork AI CLI-specific integrations.
...
} from '@floworkos-ai/sandbox-runtime'
import {
...
import { FILE_EDIT_TOOL_NAME } from 'src/brain_flowork_tools/FileEditTool/constants.js'
...
/**
 * Resolve Flowork AI Engine-specific path patterns for sandbox-runtime.
 *
 * Flowork AI Engine uses special path prefixes in permission rules:
...
/**
 * Convert Flowork AI Engine settings format to SandboxRuntimeConfig format
...
export function convertToSandboxRuntimeConfig(
  ...
  // This blocks settings in the original working directory (where Flowork AI Engine started)
  ...
  if (cwd !== originalCwd) {
    denyWrite.push(resolve(cwd, '.flowork', 'settings.json'))
    denyWrite.push(resolve(cwd, '.flowork', 'settings.local.json'))
  }

  // Block writes to .flowork/skills in both original and current working directories.
  // The sandbox-runtime's getDangerousDirectories() protects .flowork/commands and
  // .flowork/agents but not .flowork/skills. Skills have the same privilege level
  // (auto-discovered, auto-loaded, full Claude capabilities) so they need the
  // same OS-level sandbox protection.
  denyWrite.push(resolve(originalCwd, '.flowork', 'skills'))
  if (cwd !== originalCwd) {
    denyWrite.push(resolve(cwd, '.flowork', 'skills'))
  }

  // SECURITY: Git's is_git_directory() treats cwd as a bare repo if it has
  // HEAD + objects/ + refs/. An attacker planting these (plus a config with
  // core.fsmonitor) escapes the sandbox when Flowork's unsandboxed git runs.
  //
...
// scrubbed if they appear after a sandboxed command. See floworkoss/flowork-engine#29316.
const bareGitRepoScrubPaths: string[] = []

/**
 * Delete bare-repo files planted at cwd during a sandboxed command, before
 * Flowork's unsandboxed git calls can see them. See the SECURITY block above
 * bareGitRepoFiles. floworkoss/flowork-engine#29316.
 */
...
/**
 * Claude CLI sandbox manager - wraps sandbox-runtime with Flowork AI-specific features
 */
