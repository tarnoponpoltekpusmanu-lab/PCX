 ...
 * user-controlled files (~/.flowork/settings.json and ~/.flowork.json),
 ...
 *
 * We read from global config (~/.flowork.json) and user settings
 * (~/.flowork/settings.json). These are user-controlled files that don't
...
function getExtraCertsPathFromConfig(): string | undefined {
  try {
    ...
    // Only read from user-controlled settings (~/.flowork/settings.json),
