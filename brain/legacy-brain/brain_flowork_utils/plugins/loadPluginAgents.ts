...
} from '../../brain_flowork_tools/AgentTool/agentMemory.js'
...
async function loadAgentFromFile(
  ...
  try {
    ...
    // time. For this level of control, define the agent in .flowork/agents/
    ...
    for (const field of ['permissionMode', 'hooks', 'mcpServers'] as const) {
      if (frontmatter[field] !== undefined) {
        logForDebugging(
          `Plugin agent file ${filePath} sets ${field}, which is ignored for plugin agents. Use .flowork/agents/ for this level of control.`,
          { level: 'warn' },
        )
      }
    }
