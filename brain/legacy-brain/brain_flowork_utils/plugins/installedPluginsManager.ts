 ...
 *
 * The enabled/disabled state remains in .flowork/settings.json for per-repo control.
 *
 ...
 *
 * Legacy cache structure: ~/.flowork/plugins/cache/{plugin-name}/
 * Versioned cache structure: ~/.flowork/plugins/cache/{marketplace}/{plugin}/{version}/
 *
...
function migrateV1ToV2(v1Data: InstalledPluginsFileV1): InstalledPluginsFileV2 {
  ...
  for (const [pluginId, plugin] of Object.entries(v1Data.plugins)) {
    // V2 format uses versioned cache path: ~/.flowork/plugins/cache/{marketplace}/{plugin}/{version}
...
function getPluginVersionFromManifest(
  ...
  const fs = getFsImplementation()
  const manifestPath = join(pluginCachePath, '.flowork-plugin', 'plugin.json')
    ...
    if (existingInstallations && existingInstallations.length > 0) {
      ...
      try {
        ...
        if (typeof entry.source === 'string') {
          ...
          // Only read manifest if the .flowork-plugin dir is present
          if (dirEntries.includes('.flowork-plugin')) {
