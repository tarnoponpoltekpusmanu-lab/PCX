 ...
 * When FLOWORK_ENGINE_PLUGIN_USE_ZIP_CACHE is enabled and FLOWORK_ENGINE_PLUGIN_CACHE_DIR
...
export function isPluginZipCacheEnabled(): boolean {
  return isEnvTruthy(process.env.FLOWORK_ENGINE_PLUGIN_USE_ZIP_CACHE)
}

/**
 * Get the path to the zip cache directory.
 * Requires FLOWORK_ENGINE_PLUGIN_CACHE_DIR to be set.
...
export function getPluginZipCachePath(): string | undefined {
  ...
  const dir = process.env.FLOWORK_ENGINE_PLUGIN_CACHE_DIR
...
async function collectFilesForZip(
  ...
  // See also: markdownConfigLoader.ts getFileIdentity, floworkoss/flowork-engine#13893
  try {
