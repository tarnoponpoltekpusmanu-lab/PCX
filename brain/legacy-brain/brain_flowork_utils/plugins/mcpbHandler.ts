...
export function saveMcpServerUserConfig(
  ...
  try {
    ...
    if (
      ...
      // pluginOptionsStorage.ts:184, see FLOWORK.md's 10% case).
...
async function generateMcpConfig(
  ...
  // Lazy import: @floworkos-ai/mcpb barrel pulls in zod v3 schemas (~700KB of
  // bound closures). See dxt/helpers.ts for details.
  const { getMcpConfigForManifest } = await import('@floworkos-ai/mcpb')
