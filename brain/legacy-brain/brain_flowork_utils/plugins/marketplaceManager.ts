/**
 * Marketplace manager for Flowork AI Engine plugins
 *
 ...
 *
 * Reads the configuration file at ~/.flowork/plugins/known_marketplaces.json
 ...
 *     "installLocation": "/Users/me/.flowork/plugins/marketplaces/official-marketplace.json",
 *     "lastUpdated": "2024-01-15T10:30:00.000Z"
 *   },
 *   "company-plugins": {
 *     "source": { "source": "github", "repo": "mycompany/plugins" },
 *     "installLocation": "/Users/me/.flowork/plugins/marketplaces/company-plugins",
...
export async function loadKnownMarketplacesConfigSafe(): Promise<KnownMarketplacesConfig> {
  try {
    ...
    // corrupted user config isn't a Flowork AI Engine bug, shouldn't hit the error file.
    return {}
  }
}
 ...
 *
 * Writes the configuration to ~/.flowork/plugins/known_marketplaces.json,
 ...
 *
 * Pulls latest changes with a configurable timeout (default 120s, override via FLOWORK_ENGINE_PLUGIN_GIT_TIMEOUT_MS).
...
function getPluginGitTimeoutMs(): number {
  const envValue = process.env.FLOWORK_ENGINE_PLUGIN_GIT_TIMEOUT_MS
  if (envValue) {
...
function enhanceGitPullErrorMessages(result: {
  ...
  if (result.error?.includes('timed out')) {
    ...
    return {
      ...result,
      stderr: `Git pull timed out after ${timeoutSec}s. Try increasing the timeout via FLOWORK_ENGINE_PLUGIN_GIT_TIMEOUT_MS environment variable.\n\nOriginal error: ${result.stderr}`,
    }
  }
 ...
 *
 * Clones a git repository with a configurable timeout (default 120s, override via FLOWORK_ENGINE_PLUGIN_GIT_TIMEOUT_MS)
...
export async function gitClone(
  ...
  if (result.error?.includes('timed out')) {
    return {
      ...result,
      stderr: `Git clone timed out after ${Math.round(timeoutMs / 1000)}s. The repository may be too large for the current timeout. Set FLOWORK_ENGINE_PLUGIN_GIT_TIMEOUT_MS to increase it (e.g., 300000 for 5 minutes).\n\nOriginal error: ${result.stderr}`,
    }
  }
 ...
 * - GitHub: Clones repo and looks for .flowork-plugin/marketplace.json
    ...
    switch (source.source) {
      ...
      case 'github': {
        ...
        marketplacePath = join(
          temporaryCachePath,
          source.path || '.flowork-plugin/marketplace.json',
        )
        break
      }
      ...
      case 'file': {
        ...
        // File sources point to .flowork-plugin/marketplace.json, so the marketplace
        // root is two directories up (parent of .flowork-plugin/)
      ...
      case 'directory': {
        // For directories, look for .flowork-plugin/marketplace.json
        // Resolve to absolute so error messages show the actual path checked
        // (legacy known_marketplaces.json entries may have relative paths)
        const absPath = resolve(source.path)
        marketplacePath = join(absPath, '.flowork-plugin', 'marketplace.json')
 ...
 * The configuration is saved to ~/.flowork/plugins/known_marketplaces.json.
 *
...
async function readCachedMarketplace(
  ...
  // For git-sourced directories, the manifest lives at .flowork-plugin/marketplace.json.
  ...
  const nestedPath = join(installLocation, '.flowork-plugin', 'marketplace.json')
  try {
    ...
    if (
      ...
      throw new Error(
        `Marketplace "${name}" has a relative source path (${entry.source.path}) ` +
          ...
          `Flowork AI Engine version. Run 'claude marketplace remove ${name}' and ` +
  ...
  try {
    ...
    if (source.source === 'github' || source.source === 'git') {
      ...
      if (source.source === 'github') {
        ...
        if (isEnvTruthy(process.env.FLOWORK_ENGINE_REMOTE)) {
      ...
      try {
        ...
        const reason =
          name === 'flowork-engine-plugins'
            ? `We've deprecated "flowork-engine-plugins" in favor of "claude-plugins-official".`
