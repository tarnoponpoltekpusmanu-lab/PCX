 ...
 *
 * This module is responsible for discovering, loading, and validating Flowork AI Engine plugins
...
import { getInlinePlugins } from '../../brain_flowork_bootstrap/state.js'
import {
  BUILTIN_MARKETPLACE_NAME,
  getBuiltinPlugins,
} from '../../brain_flowork_plugins/builtinPlugins.js'
import type {
  LoadedPlugin,
  PluginComponent,
  PluginError,
  PluginLoadResult,
  PluginManifest,
} from '../../brain_flowork_types/plugin.js'
 ...
 * Format: ~/.flowork/plugins/cache/{marketplace}/{plugin}/{version}/
 *
 * @param pluginId - Plugin identifier in format "name@marketplace"
 * @param version - Version string (semver, git SHA, etc.)
 ...
 * Format: ~/.flowork/plugins/cache/{plugin-name}/
 *
...
async function installFromGitHub(
  ...
  const gitUrl = isEnvTruthy(process.env.FLOWORK_ENGINE_REMOTE)
 ...
 * FLOWORK_ENGINE_REMOTE) or any URL that passes validateGitUrl (https, http,
 * file, git@ ssh).
 */
...
export async function cachePlugin(
  ...
  const manifestPath = join(tempPath, '.flowork-plugin', 'plugin.json')
 ...
 * @param source - Source description for default manifest (e.g., "git:repo" or ".flowork-plugin/name")
 ...
 * @param source - Source identifier (e.g., "git:repo", ".flowork-plugin/my-plugin")
...
export async function createPluginFromPath(
  ...
  const manifestPath = join(pluginPath, '.flowork-plugin', 'plugin.json')
  ...
  const plugin: LoadedPlugin = {
    ...
    source, // Source identifier (e.g., "git:repo" or ".flowork-plugin/name")
 ...
 *
 * FLOWORK_ENGINE_SYNC_PLUGIN_INSTALL=1 delegates to the full loader — that
 * mode explicitly opts into blocking install before first query, and
 * main.tsx's getFloworkEngineMcpConfigs()/getInitialSettings().agent run
 ...
 * - After modifying .flowork-plugin/ directory (for export)
