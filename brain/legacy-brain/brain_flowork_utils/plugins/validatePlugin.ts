...
function detectManifestType(
  ...
  // Check if it's in .flowork-plugin directory
  if (dirName === '.flowork-plugin') {
 ...
 * '..' a user added to "climb out of .flowork-plugin/" is unnecessary. Callers pass `hint`
...
// they expect paths to resolve relative to marketplace.json (inside .flowork-plugin/),
...
function marketplaceSourceHint(p: string): string {
  ...
  // .flowork-plugin/" is unnecessary since paths already start at the repo root.
  ...
  return (
    ...
    'containing .flowork-plugin/), not relative to marketplace.json. ' +
      ...
      for (const key of strayKeys) {
        ...
        warnings.push({
          ...
          message:
            ...
            `not plugin.json. It's harmless here but unused — Flowork AI Engine ` +
  ...
  if (result.success) {
    ...
    if (!/^[a-z0-9]+(-[a-z0-9]+)*$/.test(manifest.name)) {
      warnings.push({
        path: 'name',
        message:
          `Plugin name "${manifest.name}" is not kebab-case. Flowork AI Engine accepts ` +
  ...
  if (result.success) {
    ...
    if (marketplace.plugins) {
      ...
      const marketplaceRoot =
        path.basename(manifestDir) === '.flowork-plugin'
      ...
      for (const [i, entry] of marketplace.plugins.entries()) {
        ...
        if (manifestVersion && manifestVersion !== entry.version) {
          warnings.push({
            ...
            message:
              `Entry declares version "${entry.version}" but ${entry.source}/.flowork-plugin/plugin.json says "${manifestVersion}". ` +
...
export async function validateManifest(
  ...
  if (stats?.isDirectory()) {
    // Look for manifest files in .flowork-plugin directory
    ...
    const pluginPath = path.join(absolutePath, '.flowork-plugin', 'plugin.json')
    ...
    return {
      ...
      errors: [
        {
          path: 'directory',
          message: `No manifest found in directory. Expected .flowork-plugin/marketplace.json or .flowork-plugin/plugin.json`,
        },
      ],
      warnings: [],
      filePath: absolutePath,
