 ...
 * - CLI flag: --cowork
 * - Environment variable: FLOWORK_ENGINE_USE_COWORK_PLUGINS
 *
 * The base directory can be overridden via FLOWORK_ENGINE_PLUGIN_CACHE_DIR.
 */

import { mkdirSync } from 'fs'
import { readdir, rm, stat } from 'fs/promises'
import { delimiter, join } from 'path'
import { getUseCoworkPlugins } from '../../brain_flowork_bootstrap/state.js'
 ...
 * 2. Environment variable FLOWORK_ENGINE_USE_COWORK_PLUGINS
 * 3. Default: 'plugins'
 */
function getPluginsDirectoryName(): string {
  ...
  if (isEnvTruthy(process.env.FLOWORK_ENGINE_USE_COWORK_PLUGINS)) {
    return COWORK_PLUGINS_DIR
  }
  return PLUGINS_DIR
}
 ...
 *
 * Priority:
 * 1. FLOWORK_ENGINE_PLUGIN_CACHE_DIR env var (explicit override)
 * 2. Default: ~/.flowork/plugins or ~/.flowork/cowork_plugins
 */
export function getPluginsDirectory(): string {
  // expandTilde: when FLOWORK_ENGINE_PLUGIN_CACHE_DIR is set via settings.json
  // `env` (not shell), ~ is not expanded by the shell. Without this, a value
  // like "~/.flowork/plugins" becomes a literal `~` directory created in the
  // cwd of every project (gh-30794 / CC-212).
  const envOverride = process.env.FLOWORK_ENGINE_PLUGIN_CACHE_DIR
  if (envOverride) {
 ...
 * image and point FLOWORK_ENGINE_PLUGIN_SEED_DIR at it. CC will use it as a
 ...
 *   $FLOWORK_ENGINE_PLUGIN_SEED_DIR/
...
export function getPluginSeedDirs(): string[] {
  ...
  const raw = process.env.FLOWORK_ENGINE_PLUGIN_SEED_DIR
  if (!raw) return []
