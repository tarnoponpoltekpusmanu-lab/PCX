/**
 * Perfetto Tracing for Flowork AI Engine (Ant-only)
 *
 ...
 *
 * Usage:
 * 1. Enable via FLOWORK_ENGINE_PERFETTO_TRACE=1 or FLOWORK_ENGINE_PERFETTO_TRACE=<path>
 * 2. Optionally set FLOWORK_ENGINE_PERFETTO_WRITE_INTERVAL_S=<positive integer> to write the
 *    trace file periodically (default: write only on exit).
 * 3. Run Flowork AI Engine normally
 * 4. Trace file is written to ~/.flowork/traces/trace-<session-id>.json
...
export function initializePerfettoTracing(): void {
  const envValue = process.env.FLOWORK_ENGINE_PERFETTO_TRACE
  logForDebugging(
  ...
  if (feature('PERFETTO_TRACING')) {
    ...
    // Start periodic full-trace write if FLOWORK_ENGINE_PERFETTO_WRITE_INTERVAL_S is a positive integer
    const intervalSec = parseInt(
      process.env.FLOWORK_ENGINE_PERFETTO_WRITE_INTERVAL_S ?? '',
      10,
    )
    if (intervalSec > 0) {
