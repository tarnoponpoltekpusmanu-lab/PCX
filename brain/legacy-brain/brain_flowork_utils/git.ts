...
import { hasBinaryExtension, isBinaryContent } from '../brain_flowork_constants/files.js'
...
const resolveCanonicalRoot = memoizeWithLRU(
  (gitRoot: string): string => {
    try {
      ...
      // dialog and executing hooks from .flowork/settings.json on startup.
      //
      ...
      // Use the common dir itself as the stable identity (floworkoss/flowork-engine#27994).
...
export const stashToCleanState = async (message?: string): Promise<boolean> => {
  try {
    const stashMessage =
      message || `Flowork AI Engine auto-stash - ${new Date().toISOString()}`
