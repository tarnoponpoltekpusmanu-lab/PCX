...
import type { Tool, ToolPermissionContext } from '../brain_flowork_tool.js'
...
async function checkClaudeMdFiles(): Promise<ContextWarning | null> {
  ...
  const message =
    largeFiles.length === 1
      ? `Large FLOWORK.md file detected (${largeFiles[0]!.content.length.toLocaleString()} chars > ${MAX_MEMORY_CHARACTER_COUNT.toLocaleString()})`
      : `${largeFiles.length} large FLOWORK.md files detected (each > ${MAX_MEMORY_CHARACTER_COUNT.toLocaleString()} chars)`
