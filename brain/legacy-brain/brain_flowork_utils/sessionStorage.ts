...
// imports above. Named (not wildcard) per FLOWORK.md style; no collisions
...
import { builtInCommandNames } from '../brain_flowork_commands.js'
import { COMMAND_NAME_TAG, TICK_TAG } from '../brain_flowork_constants/xml.js'
import { getFeatureValue_CACHED_MAY_BE_STALE } from '../brain_flowork_services/analytics/growthbook.js'
import * as sessionIngress from '../brain_flowork_services/api/sessionIngress.js'
import { REPL_TOOL_NAME } from '../brain_flowork_tools/REPLTool/constants.js'
import {
  type AgentId,
  asAgentId,
  asSessionId,
  type SessionId,
} from '../brain_flowork_types/ids.js'
import type { AttributionSnapshotMessage } from '../brain_flowork_types/logs.js'
import {
...
import type { QueueOperationMessage } from '../brain_flowork_types/messageQueueTypes.js'
...
function getEntrypoint(): string | undefined {
  return process.env.FLOWORK_ENGINE_ENTRYPOINT
}
...
class Project {
   ...
   * FLOWORK_ENGINE_SKIP_PROMPT_HISTORY should suppress all transcript writes.
  ...
  private shouldSkipPersistence(): boolean {
    ...
    return (
      ...
      isSessionPersistenceDisabled() ||
      isEnvTruthy(process.env.FLOWORK_ENGINE_SKIP_PROMPT_HISTORY)
    )
  }
...
export async function loadTranscriptFile(
  ...
  try {
    ...
    if (!isEnvTruthy(process.env.FLOWORK_ENGINE_DISABLE_PRECOMPACT_SKIP)) {
    ...
    // FLOWORK_ENGINE_DISABLE_PRECOMPACT_SKIP is set (that kill switch means
    ...
    if (
      !opts?.keepAllLeaves &&
      !hasPreservedSegment &&
      !isEnvTruthy(process.env.FLOWORK_ENGINE_DISABLE_PRECOMPACT_SKIP) &&
...
export function isLoggableMessage(m: Message): boolean {
  ...
  if (m.type === 'attachment' && getUserType() !== 'ant') {
    if (
      ...
      isEnvTruthy(process.env.FLOWORK_ENGINE_SAVE_HOOK_ADDITIONAL_CONTEXT)
    ) {
      return true
    }
    return false
