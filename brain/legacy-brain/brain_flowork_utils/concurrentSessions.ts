...
function envSessionKind(): SessionKind | undefined {
  if (feature('BG_SESSIONS')) {
    const k = process.env.FLOWORK_ENGINE_SESSION_KIND
  ...
  try {
    ...
    await writeFile(
      ...
      jsonStringify({
        ...
        ...(feature('UDS_INBOX')
          ? { messagingSocketPath: process.env.FLOWORK_ENGINE_MESSAGING_SOCKET }
          : {}),
        ...(feature('BG_SESSIONS')
          ? {
              name: process.env.FLOWORK_ENGINE_SESSION_NAME,
              logPath: process.env.FLOWORK_ENGINE_SESSION_LOG,
              agent: process.env.FLOWORK_ENGINE_AGENT,
            }
          : {}),
      }),
    )
...
export async function countConcurrentSessions(): Promise<number> {
  ...
  for (const file of files) {
    ...
    // See floworkoss/flowork-engine#34210.
    ...
    if (isProcessRunning(pid)) {
      ...
      // ~/.flowork/sessions/ is shared with Windows-native Claude (symlink
