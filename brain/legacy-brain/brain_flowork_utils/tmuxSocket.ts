 ...
 * =====================
 * This module manages an isolated tmux socket for Flowork's operations.
 *
 * WHY THIS EXISTS:
 ...
 * directly or via Bash - will operate on Flowork's isolated socket, NOT the
 * user's tmux session.
 *
...
const CLAUDE_SOCKET_PREFIX = 'flowork'
...
/**
 * Gets the socket name for Flowork's isolated tmux session.
 * Format: claude-<PID>
 */
...
/**
 * Gets the TMUX environment variable value for Flowork's isolated socket.
 *
 ...
 * the Bash tool will operate on Flowork's socket, NOT the user's session.
 *
...
/**
 * Kills the tmux server for Flowork's isolated socket.
...
async function doInitialize(): Promise<void> {
  ...
  // Pass FLOWORK_ENGINE_SKIP_PROMPT_HISTORY via -e so it's set in the initial shell environment
  //
  ...
  const result = await execTmux([
    ...
    'base',
    '-e',
    'FLOWORK_ENGINE_SKIP_PROMPT_HISTORY=true',
    ...(getPlatform() === 'windows'
  ...
  // Set FLOWORK_ENGINE_SKIP_PROMPT_HISTORY in the tmux GLOBAL environment (-g).
  ...
  // Any Flowork AI Engine instance spawned on this socket will inherit this env var,
  ...
  await execTmux([
    ...
    'set-environment',
    '-g',
    'FLOWORK_ENGINE_SKIP_PROMPT_HISTORY',
    'true',
  ])
