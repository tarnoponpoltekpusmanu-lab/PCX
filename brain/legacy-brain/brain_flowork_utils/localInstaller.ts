...
export function getLocalClaudePath(): string {
  return join(getLocalInstallDir(), 'flowork')
}
...
export function isRunningFromLocalInstallation(): boolean {
  ...
  return execPath.includes('/.flowork/local/node_modules/')
}
...
export async function ensureLocalPackageEnvironment(): Promise<boolean> {
  try {
    ...
    const wrapperPath = join(localInstallDir, 'flowork')
    const created = await writeIfMissing(
      wrapperPath,
      `#!/bin/sh\nexec "${localInstallDir}/node_modules/.bin/flowork" "$@"`,
      0o755,
    )
    if (created) {
...
export async function localInstallationExists(): Promise<boolean> {
  try {
    await access(join(getLocalInstallDir(), 'node_modules', '.bin', 'flowork'))
    return true
  } catch {
    return false
  }
