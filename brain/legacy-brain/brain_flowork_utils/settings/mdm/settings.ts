/**
 * MDM (Mobile Device Management) profile enforcement for Flowork AI Engine managed settings.
 *
 * Reads enterprise settings from OS-level MDM configuration:
 * - macOS: `com.floworkos.floworkcode` preference domain
 *   (MDM profiles at /Library/Managed Preferences/ only — not user-writable ~/Library/Preferences/)
 * - Windows: `HKLM\SOFTWARE\Policies\FloworkEngine` (admin-only)
 *   and `HKCU\SOFTWARE\Policies\FloworkEngine` (user-writable, lowest priority)
 * - Linux: No MDM equivalent (uses /etc/flowork-engine/managed-settings.json instead)
 *
