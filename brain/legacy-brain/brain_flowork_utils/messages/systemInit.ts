...
import { getSdkBetas, getSessionId } from 'src/brain_flowork_bootstrap/state.js'
import { DEFAULT_OUTPUT_STYLE_NAME } from 'src/brain_flowork_constants/outputStyles.js'
import type {
  ApiKeySource,
  PermissionMode,
  SDKMessage,
...
} from 'src/brain_flowork_tools/AgentTool/constants.js'
import { getFloworkOSApiKeyWithSource } from '../auth.js'
...
export function buildSystemInitMessage(inputs: SystemInitInputs): SDKMessage {
  ...
  const initMessage: SDKMessage = {
    ...
    slash_commands: inputs.commands
      ...
      .map(c => c.name),
    apiKeySource: getFloworkOSApiKeyWithSource().source as ApiKeySource,
    betas: getSdkBetas(),
