...
import { startsWithApiErrorPrefix } from '../../brain_flowork_services/api/errors.js'
...
async function getCommandPrefixImpl(
  ...
  try {
    ...
    preflightCheckTimeoutId = setTimeout(
      (tn, nonInteractive) => {
        const message = `[${tn}Tool] Pre-flight check is taking longer than expected. Run with FLOWORKOS_LOG=debug to check for failed or slow API requests.`
        if (nonInteractive) {
