// Scheduler lease lock for .flowork/scheduled_tasks.json.
//
...
const LOCK_FILE_REL = join('.flowork', 'scheduled_tasks.lock')
...
async function tryCreateExclusive(
  ...
  try {
    ...
    if (code === 'ENOENT') {
      // .flowork/ doesn't exist yet — create it and retry once. In steady
