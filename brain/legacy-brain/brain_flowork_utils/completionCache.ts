...
function detectShell(): ShellInfo | null {
  ...
  const home = homedir()
  const floworkDir = join(home, '.flowork')

  if (shell.endsWith('/zsh') || shell.endsWith('/zsh.exe')) {
    const cacheFile = join(floworkDir, 'completion.zsh')
    return {
      name: 'zsh',
  ...
  if (shell.endsWith('/bash') || shell.endsWith('/bash.exe')) {
    const cacheFile = join(floworkDir, 'completion.bash')
    return {
      name: 'bash',
  ...
  if (shell.endsWith('/fish') || shell.endsWith('/fish.exe')) {
    ...
    const cacheFile = join(floworkDir, 'completion.fish')
    return {
      name: 'fish',
...
export async function setupShellCompletion(theme: ThemeName): Promise<string> {
  ...
  const claudeBin = process.argv[1] || 'flowork'
...
/**
 * Regenerate cached shell completion scripts in ~/.flowork/.
