...
/**
 * Builds a deep link URL for Flowork Desktop to resume a CLI session.
...
function buildDesktopDeepLink(sessionId: string): string {
  const protocol = isDevMode() ? 'claude-dev' : 'flowork'
...
async function isDesktopInstalled(): Promise<boolean> {
  ...
  if (platform === 'darwin') {
    // Check for Flowork.app in /Applications
...
async function getDesktopVersion(): Promise<string | null> {
  ...
  if (platform === 'darwin') {
    ...
    const installDir = join(localAppData, 'FloworkOSClaude')
    try {
