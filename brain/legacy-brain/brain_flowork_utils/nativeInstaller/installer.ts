...
export function getBinaryName(platform: string): string {
  return platform.startsWith('win32') ? 'claude.exe' : 'flowork'
}
...
function getBaseDirectories() {
  ...
  return {
    ...
    versions: join(getXDGDataHome(), 'flowork', 'versions'),

    // Cache directories (can be deleted)
    staging: join(getXDGCacheHome(), 'flowork', 'staging'),

    // State directories
    locks: join(getXDGStateHome(), 'flowork', 'locks'),
...
async function installVersionFromPackage(
  ...
  try {
    ...
    const nodeModulesDir = join(stagingPath, 'node_modules', '@floworkos-ai')
...
async function performVersionUpdate(
  ...
  // Create direct symlink from ~/.local/bin/flowork to the version binary
...
async function manualRemoveNpmPackage(
  ...
  try {
    ...
    if (getPlatform().startsWith('win32')) {
      ...
      const binExe = join(globalPrefix, 'flowork')
      ...
      const binSymlink = join(globalPrefix, 'bin', 'flowork')
...
export async function cleanupNpmInstallations(): Promise<{
  ...
  // Always attempt to remove @floworkos-ai/flowork-engine
  ...
  // Check for local installation at ~/.flowork/local
  const localInstallDir = join(homedir(), '.flowork', 'local')
