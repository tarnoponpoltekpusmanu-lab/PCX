 ...
 *
 * This module provides PID-based locking for running Flowork AI Engine versions.
...
function isClaudeProcess(pid: number, expectedExecPath: string): boolean {
  ...
  // This handles test environments where the command might not contain 'flowork'
  if (pid === process.pid) {
    return true
  }
  ...
  try {
    ...
    // Check if the command contains 'flowork' or the expected exec path
    const normalizedCommand = command.toLowerCase()
    const normalizedExecPath = expectedExecPath.toLowerCase()

    return (
      normalizedCommand.includes('flowork') ||
