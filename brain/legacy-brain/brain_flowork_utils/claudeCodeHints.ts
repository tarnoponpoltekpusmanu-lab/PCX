/**
 * Flowork AI Engine hints protocol.
 *
 * CLIs and SDKs running under Flowork AI Engine can emit a self-closing
 * `<flowork-engine-hint />` tag to stderr (merged into stdout by the shell
 ...
 *
 * See docs/flowork-engine-hints.md for the vendor-facing spec.
 */

import { logForDebugging } from './debug.js'
import { createSignal } from './signal.js'

export type FloworkEngineHintType = 'plugin'

export type FloworkEngineHint = {
  /** Spec version declared by the emitter. Unknown versions are dropped. */
  v: number
  /** Hint discriminator. v1 defines only `plugin`. */
  type: FloworkEngineHintType
  /**
 ...
 */
const HINT_TAG_RE = /^[ \t]*<flowork-engine-hint\s+([^>]*?)\s*\/>[ \t]*$/gm
 ...
 */
export function extractFloworkEngineHints(
  output: string,
  command: string,
): { hints: FloworkEngineHint[]; stripped: string } {
  // Fast path: no tag open sequence → no work, no allocation.
  if (!output.includes('<flowork-engine-hint')) {
    return { hints: [], stripped: output }
  }

  const sourceCommand = firstCommandToken(command)
  const hints: FloworkEngineHint[] = []
  ...
  const stripped = output.replace(HINT_TAG_RE, rawLine => {
    ...
    if (!SUPPORTED_VERSIONS.has(v)) {
      logForDebugging(
        `[floworkEngineHints] dropped hint with unsupported v=${attrs.v}`,
      )
      return ''
    }
    if (!type || !SUPPORTED_TYPES.has(type)) {
      logForDebugging(
        `[floworkEngineHints] dropped hint with unsupported type=${type}`,
      )
      return ''
    }
    if (!value) {
      logForDebugging('[floworkEngineHints] dropped hint with empty value')
      return ''
    }

    hints.push({ v, type: type as FloworkEngineHintType, value, sourceCommand })
    return ''
  })
...
let pendingHint: FloworkEngineHint | null = null
...
export function setPendingHint(hint: FloworkEngineHint): void {
...
export function getPendingHintSnapshot(): FloworkEngineHint | null {
  return pendingHint
}
...
/** Test-only reset. */
export function _resetFloworkEngineHintStore(): void {
  pendingHint = null
