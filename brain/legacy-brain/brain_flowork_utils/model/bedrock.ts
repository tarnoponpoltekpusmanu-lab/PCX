...
> {
  ...
  try {
    ...
    // Filter for FloworkOS models (SYSTEM_DEFINED filtering handled in query)
    return allProfiles
      .filter(profile => profile.inferenceProfileId?.includes('floworkos'))
...
async function createBedrockClient() {
  ...
  // Match the FloworkOS Bedrock SDK's region behavior exactly:
  ...
  const skipAuth = isEnvTruthy(process.env.FLOWORK_ENGINE_SKIP_BEDROCK_AUTH)

  const clientConfig: ConstructorParameters<typeof BedrockClient>[0] = {
    region,
    ...(process.env.FLOWORKOS_BEDROCK_BASE_URL && {
      endpoint: process.env.FLOWORKOS_BEDROCK_BASE_URL,
    }),
...
/**
 * Check if a model ID is a foundation model (e.g., "floworkos.flowork-sonnet-4-5-20250929-v1:0")
 */
export function isFoundationModel(modelId: string): boolean {
  return modelId.startsWith('floworkos.')
}
 ...
 * For example:
 * - "eu.floworkos.flowork-sonnet-4-5-20250929-v1:0" → "eu"
 * - "us.floworkos.flowork-3-7-sonnet-20250219-v1:0" → "us"
 * - "arn:aws:bedrock:ap-northeast-2:123:inference-profile/global.floworkos.flowork-opus-4-6-v1" → "global"
 * - "floworkos.flowork-3-5-sonnet-20241022-v2:0" → undefined (foundation model)
...
export function getBedrockRegionPrefix(
  ...
  for (const prefix of BEDROCK_REGION_PREFIXES) {
    if (effectiveModelId.startsWith(`${prefix}.floworkos.`)) {
      return prefix
    }
  }
  return undefined
 ...
 * If the model is a foundation model (floworkos.*), the prefix will be added.
 * If the model is not a Bedrock model, it will be returned as-is.
 *
 * For example:
 * - applyBedrockRegionPrefix("us.floworkos.flowork-sonnet-4-5-v1:0", "eu") → "eu.floworkos.flowork-sonnet-4-5-v1:0"
 * - applyBedrockRegionPrefix("floworkos.flowork-sonnet-4-5-v1:0", "eu") → "eu.floworkos.flowork-sonnet-4-5-v1:0"
...
export function applyBedrockRegionPrefix(
  ...
  // Check if it's a foundation model (floworkos.*) and add the prefix
