...
import { getAPIProvider, isFirstPartyFloworkOSBaseUrl } from './providers.js'
...
function isModelCapabilitiesEligible(): boolean {
  ...
  if (!isFirstPartyFloworkOSBaseUrl()) return false
  return true
}
...
export async function refreshModelCapabilities(): Promise<void> {
  ...
  try {
    const floworkos = await getFloworkOSClient({ maxRetries: 1 })
    const betas = isClaudeAISubscriber() ? [OAUTH_BETA_HEADER] : undefined
    const parsed: ModelCapability[] = []
    for await (const entry of floworkos.models.list({ betas })) {
