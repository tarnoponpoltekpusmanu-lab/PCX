...
import {
  type FilesApiConfig,
  uploadSessionFiles,
} from '../../brain_flowork_services/api/filesApi.js'
 ...
 * - Checks environment kind (FLOWORK_ENGINE_ENVIRONMENT_KIND)
 * - Retrieves session access token
 * - Requires FLOWORK_ENGINE_REMOTE_SESSION_ID for session ID
 *
...
export async function runFilePersistence(
  ...
  if (!sessionId) {
    logError(
      new Error(
        'File persistence enabled but FLOWORK_ENGINE_REMOTE_SESSION_ID is not set',
      ),
    )
    return null
  }
 ...
 * and FLOWORK_ENGINE_REMOTE_SESSION_ID.
 * This ensures only public-api/sessions users trigger file persistence,
 * not normal Flowork AI Engine CLI users.
 */
