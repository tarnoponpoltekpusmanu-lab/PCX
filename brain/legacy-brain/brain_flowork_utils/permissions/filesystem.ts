...
import { hasAutoMemPathOverride, isAutoMemPath } from 'src/brain_flowork_memdir/paths.js'
import { isAgentMemoryPath } from 'src/brain_flowork_tools/AgentTool/agentMemory.js'
import {
  CLAUDE_FOLDER_PERMISSION_PATTERN,
  FILE_EDIT_TOOL_NAME,
  GLOBAL_CLAUDE_FOLDER_PERMISSION_PATTERN,
} from 'src/brain_flowork_tools/FileEditTool/constants.js'
import type { z } from 'zod/v4'
import { getOriginalCwd, getSessionId } from '../../brain_flowork_bootstrap/state.js'
import { checkStatsigFeatureGate_CACHED_MAY_BE_STALE } from '../../brain_flowork_services/analytics/growthbook.js'
import type { AnyObject, Tool, ToolPermissionContext } from '../../brain_flowork_tool.js'
...
/**
 * If filePath is inside a .flowork/skills/{name}/ directory (project or global),
 ...
 * require granting session access to all of .flowork/ (settings.json, hooks/, etc.).
 */
export function getClaudeSkillScope(
  ...
  const bases = [
    {
      dir: expandPath(join(getOriginalCwd(), '.flowork', 'skills')),
      prefix: '/.flowork/skills/',
    },
    {
      dir: expandPath(join(homedir(), '.flowork', 'skills')),
      prefix: '~/.flowork/skills/',
    },
  ]
  ...
  for (const { dir, prefix } of bases) {
    ...
    for (const s of [sep, '/']) {
      if (absolutePathLower.startsWith(dirLower + s.toLowerCase())) {
        ...
        // produce '/.flowork/skills/*/**' which matches ALL skills. Return null
...
export function isClaudeSettingsPath(filePath: string): boolean {
  ...
  // sequences like `./.flowork/./settings.json` which would evade the endsWith() check
  ...
  if (
    normalizedPath.endsWith(`${sep}.flowork${sep}settings.json`) ||
    normalizedPath.endsWith(`${sep}.flowork${sep}settings.local.json`)
  ) {
    // Include .flowork/settings.json even for other projects
    return true
  }
...
// Always ask when Flowork AI Engine tries to edit its own config files
function isClaudeConfigFilePath(filePath: string): boolean {
  if (isClaudeSettingsPath(filePath)) {
    return true
  }

  // Check if file is within .flowork/commands or .flowork/agents directories
  // using proper path segment validation (not string matching with includes())
  // pathInWorkingPath now handles case-insensitive comparison to prevent bypasses
  const commandsDir = join(getOriginalCwd(), '.flowork', 'commands')
  const agentsDir = join(getOriginalCwd(), '.flowork', 'agents')
  const skillsDir = join(getOriginalCwd(), '.flowork', 'skills')
 ...
 * Path format: ~/.flowork/projects/{sanitized-cwd}/...
 */
 ...
 * The scratchpad is a per-session directory for Flowork to write temporary files.
 ...
 * On Windows: 'flowork' (tmpdir() is already per-user)
 */
 ...
 * This is a per-user temporary directory used by Flowork AI Engine for all temp files.
 *
...
// and per-turn from BashTool prompt. Inputs (FLOWORK_ENGINE_TMPDIR env + platform) are
...
export const getClaudeTempDir = memoize(function getClaudeTempDir(): string {
  const baseTmpDir =
    process.env.FLOWORK_ENGINE_TMPDIR ||
...
function isDangerousFilePathToAutoEdit(path: string): boolean {
  ...
  for (let i = 0; i < pathSegments.length; i++) {
    ...
    for (const dir of DANGEROUS_DIRECTORIES) {
      ...
      // Special case: .flowork/worktrees/ is a structural path (where Claude stores
      // git worktrees), not a user-created dangerous directory. Skip the .flowork
      // segment when it's followed by 'worktrees'. Any nested .flowork directories
      ...
      if (dir === '.flowork') {
        ...
        if (
          ...
          break // Skip this .flowork, continue checking other segments
        }
      }
 ...
 * - Trailing dots and spaces (e.g., .git., .flowork , .bashrc...)
...
function hasSuspiciousWindowsPathPattern(path: string): boolean {
  ...
  // Examples: .git., .flowork , .bashrc..., settings.json.
 ...
 * - Claude config files (.flowork/settings.json, .flowork/commands/, .flowork/agents/)
 * - MCP CLI state files (managed internally by Flowork AI Engine)
...
export function checkPathSafetyForAutoEdit(
  ...
  // Check for Flowork config files on all paths
  for (const pathToCheck of pathsToCheck) {
...
function patternWithRoot(
  ...
  if (pattern.startsWith(`${DIR_SEP}${DIR_SEP}`)) {
    ...
    // Patterns starting with / resolve relative to the directory where settings are stored (without .flowork/)
    return {
...
export function checkWritePermissionForTool<Input extends AnyObject>(
  ...
  // This MUST come before isDangerousFilePathToAutoEdit check since .flowork is a dangerous directory
  ...
  // 1.6. Check for .flowork/** allow rules BEFORE safety checks
  // This allows session-level permissions to bypass the safety blocks for .flowork/
  // We only allow this for session-level rules to prevent users from accidentally
  // permanently granting broad access to their .flowork/ folder.
  //
  // matchingRuleForInput returns the first match across all sources. If the user
  // also has a broader Edit(.flowork) rule in userSettings (e.g. from sandbox
  ...
  if (claudeFolderAllowRule) {
    // Check if this rule is scoped under .flowork/ (project or global).
    // Accepts both the broad patterns ('/.flowork/**', '~/.flowork/**') and
    // narrowed ones like '/.flowork/skills/my-skill/**' so users can grant
    ...
    // '/.flowork/../**' from leaking this bypass outside .flowork/.
  ...
  if (!safetyCheck.safe) {
    // SDK suggestion: if under .flowork/skills/{name}/, emit the narrowed
    // session-scoped addRules that step 1.6 will honor on the next call.
    // Everything else (.flowork/settings.json, .git/, .vscode/, .idea/) falls
...
export function checkEditableInternalPath(
  ...
  // var value must itself resolve under ~/.flowork/jobs/. Symlink guard: every
  ...
  if (feature('TEMPLATES')) {
    ...
    if (jobDir) {
      ...
      // the case where ~/.flowork is a symlink (e.g. to /data/claude-config).
  ...
  // ~/.flowork/, which is in DANGEROUS_DIRECTORIES. The CLAUDE_COWORK_MEMORY_PATH_OVERRIDE
  ...
  // .flowork/launch.json — desktop preview config (dev server command + port).
  ...
  // .flowork/ DANGEROUS_DIRECTORIES check prompts for it, which in SDK mode
  ...
  // .flowork/ only (not ~/.flowork/) since launch.json is per-project.
  if (
    normalizeCaseForComparison(normalizedPath) ===
    normalizeCaseForComparison(join(getOriginalCwd(), '.flowork', 'launch.json'))
  ) {
    return {
      behavior: 'allow',
      updatedInput: input,
...
export function checkReadableInternalPath(
  ...
  // Tasks directory (~/.flowork/tasks/) for swarm task coordination
  ...
  // Teams directory (~/.flowork/teams/) for swarm coordination
