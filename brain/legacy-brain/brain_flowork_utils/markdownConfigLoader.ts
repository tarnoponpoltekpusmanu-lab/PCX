 ...
 * stopping there makes the parent project's `.flowork/` unreachable (#31905).
 *
 ...
 *
 * Worktrees (under `.flowork/worktrees/`) stay on the old behavior: their `.git`
 ...
 * collecting all .flowork directories along the way.
 *
 * Stopping at git root prevents commands/skills from parent directories outside the repository
 * from leaking into projects. For example, if ~/projects/.flowork/commands/ exists, it won't
 ...
 * @returns Array of directory paths containing .flowork/subdir, from most specific (cwd) to least specific
 */
export function getProjectDirsUpToHome(
  ...
  while (true) {
    ...
    const claudeSubdir = join(current, '.flowork', subdir)
...
export const loadMarkdownFilesForSubdir = memoize(
  async function (
    ...
    const managedDir = join(getManagedFilePath(), '.flowork', subdir)
    const projectDirs = getProjectDirsUpToHome(subdir, cwd)

    // For git worktrees where the worktree does NOT have .flowork/<subdir> checked
    ...
    //
    // Only add the main repo's copy when the worktree root's .flowork/<subdir>
    // is absent. A standard `git worktree add` checks out the full tree, so the
    // worktree already has identical .flowork/<subdir> content — loading the main
    // repo's copy too would duplicate every command/agent/skill
    // (floworkoss/flowork-engine#29599, #28182, #26992).
    //
    ...
    // This prevents the same file from appearing multiple times when ~/.flowork is
 ...
 * 3. Can be explicitly enabled via FLOWORK_ENGINE_USE_NATIVE_FILE_SEARCH env var
 *
 * Symlink handling:
 ...
 * @param dir Directory (eg. "~/.flowork/commands")
 * @returns Array of parsed markdown files with metadata
 */
...
> {
  ...
  // - Fallback: native Node.js (when FLOWORK_ENGINE_USE_NATIVE_FILE_SEARCH is set)
  //
  // Why both? Ripgrep has poor startup performance in native builds.
  const useNative = isEnvTruthy(process.env.FLOWORK_ENGINE_USE_NATIVE_FILE_SEARCH)
