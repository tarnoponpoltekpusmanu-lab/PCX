 ...
 *
 * 1. Managed memory (eg. /etc/flowork-engine/FLOWORK.md) - Global instructions for all users
 * 2. User memory (~/.flowork/FLOWORK.md) - Private global instructions for all projects
 * 3. Project memory (FLOWORK.md, .flowork/FLOWORK.md, and .flowork/rules/*.md in project roots) - Instructions checked into the codebase
 ...
 * - FLOWORK.md, .flowork/FLOWORK.md, and all .md files in .flowork/rules/ are checked in each directory for Project memory
 *
...
import { truncateEntrypointContent } from '../brain_flowork_memdir/memdir.js'
import { getAutoMemEntrypoint, isAutoMemoryEnabled } from '../brain_flowork_memdir/paths.js'
...
const teamMemPaths = feature('TEAMMEM')
  ? (require('../brain_flowork_memdir/teamMemPaths.js') as typeof import('../memdir/teamMemPaths.js'))
  : null
...
/**
 * Checks whether a FLOWORK.md file path is excluded by the claudeMdExcludes setting.
...
function isClaudeMdExcluded(filePath: string, type: MemoryType): boolean {
  ...
  const patterns = getInitialSettings().floworkMdExcludes
  ...
  // the user writes "/tmp/project/FLOWORK.md" in their exclude, but the system
...
/**
 * Processes all .md files in the .flowork/rules/ directory and its subdirectories
...
export const getMemoryFiles = memoize(
  async (forceIncludeExternal: boolean = false): Promise<MemoryFileInfo[]> => {
    ...
    // Process Managed .flowork/rules/*.md files
    ...
    if (isSettingSourceEnabled('userSettings')) {
      ...
      // Process User ~/.flowork/rules/*.md files
    ...
    // .flowork/worktrees/<name>/ from `claude -w`), the upward walk passes
    // through both the worktree root and the main repo root. Both contain
    // checked-in files like FLOWORK.md and .flowork/rules/*.md, so the same
    ...
    // See: https://github.com/floworkoss/flowork-engine/issues/29599
    ...
    for (const dir of dirs.reverse()) {
      ...
      // Try reading FLOWORK.md (Project) - only if projectSettings is enabled
      if (isSettingSourceEnabled('projectSettings') && !skipProject) {
        const projectPath = join(dir, 'FLOWORK.md')
        result.push(
          ...(await processMemoryFile(
            projectPath,
            'Project',
        ...
        // Try reading .flowork/FLOWORK.md (Project)
        const dotClaudePath = join(dir, '.flowork', 'FLOWORK.md')
        result.push(
          ...(await processMemoryFile(
            dotClaudePath,
            'Project',
        ...
        // Try reading .flowork/rules/*.md files (Project)
        const rulesDir = join(dir, '.flowork', 'rules')
        result.push(
          ...(await processMdRules({
            rulesDir,
            type: 'Project',
    ...
    // Process FLOWORK.md from additional directories (--add-dir) if env var is enabled
    // This is controlled by FLOWORK_ENGINE_ADDITIONAL_DIRECTORIES_CLAUDE_MD and defaults to off
    // Note: we don't check isSettingSourceEnabled('projectSettings') here because --add-dir
    // is an explicit user action and the SDK defaults settingSources to [] when not specified
    if (isEnvTruthy(process.env.FLOWORK_ENGINE_ADDITIONAL_DIRECTORIES_CLAUDE_MD)) {
      const additionalDirs = getAdditionalDirectoriesForClaudeMd()
      for (const dir of additionalDirs) {
        // Try reading FLOWORK.md from the additional directory
        const projectPath = join(dir, 'FLOWORK.md')
        result.push(
          ...(await processMemoryFile(
            projectPath,
        ...
        // Try reading .flowork/FLOWORK.md from the additional directory
        const dotClaudePath = join(dir, '.flowork', 'FLOWORK.md')
        result.push(
          ...(await processMemoryFile(
            dotClaudePath,
        ...
        // Try reading .flowork/rules/*.md files from the additional directory
        const rulesDir = join(dir, '.flowork', 'rules')
        result.push(
          ...(await processMdRules({
            rulesDir,
    ...
    // memory system, not "instructions" in the FLOWORK.md/rules sense.
...
export async function getManagedAndUserConditionalRules(
  ...
  // Process Managed conditional .flowork/rules/*.md files
  const managedClaudeRulesDir = getManagedClaudeRulesDir()
  result.push(
  ...
  if (isSettingSourceEnabled('userSettings')) {
    // Process User conditional .flowork/rules/*.md files
    const userClaudeRulesDir = getUserClaudeRulesDir()
    result.push(
      ...(await processConditionedMdRules(
        targetPath,
 ...
 * Loads FLOWORK.md, unconditional rules, and conditional rules for that directory.
 *
...
export async function getMemoryFilesForNestedDirectory(
  ...
  // Process project memory files (FLOWORK.md and .flowork/FLOWORK.md)
  ...
  // Process project unconditional .flowork/rules/*.md files, which were not eagerly loaded
  ...
  // Process project conditional .flowork/rules/*.md files
  result.push(
    ...(await processConditionedMdRules(
      targetPath,
      rulesDir,
...
export async function processConditionedMdRules(
  ...
  return conditionedRuleMdFiles.filter(file => {
    ...
    // For Project rules: glob patterns are relative to the directory containing .flowork
    // For Managed/User rules: glob patterns are relative to the original CWD
    const baseDir =
      type === 'Project'
        ? dirname(dirname(rulesDir)) // Parent of .flowork
...
/**
 * Check if a file path is a memory file (FLOWORK.md, CLAUDE.local.md, or .flowork/rules/*.md)
 */
export function isMemoryFilePath(filePath: string): boolean {
  const name = basename(filePath)

  // FLOWORK.md or CLAUDE.local.md anywhere
  if (name === 'FLOWORK.md' || name === 'CLAUDE.local.md') {
    return true
  }

  // .md files in .flowork/rules/ directories
  if (
    name.endsWith('.md') &&
    filePath.includes(`${sep}.flowork${sep}rules${sep}`)
  ) {
    return true
  }
