...
import type { ModelUsage } from 'src/brain_flowork_entrypoints/agentSdkTypes.js'
import type { Entry, TranscriptMessage } from '../brain_flowork_types/logs.js'
...
export type FloworkEngineStats = {
  // Activity overview
...
/**
 * Convert a PersistedStatsCache to FloworkEngineStats by computing derived fields.
 */
function cacheToStats(
  ...
  const result: FloworkEngineStats = {
    totalSessions,
    totalMessages,
    totalDays,
...
/**
 * Aggregates stats from all Flowork AI Engine sessions across all projects.
 * Uses a disk cache to avoid reprocessing historical data.
 */
export async function aggregateFloworkEngineStats(): Promise<FloworkEngineStats> {
 ...
 */
export async function aggregateFloworkEngineStatsForRange(
  range: StatsDateRange,
): Promise<FloworkEngineStats> {
  if (range === 'all') {
    return aggregateFloworkEngineStats()
  }
  ...
  return processedStatsToFloworkEngineStats(stats)
}

/**
 * Convert ProcessedStats to FloworkEngineStats.
 * Used for filtered date ranges that bypass the cache.
 */
function processedStatsToFloworkEngineStats(
...
function getEmptyStats(): FloworkEngineStats {
  return {
    totalSessions: 0,
    totalMessages: 0,
    totalDays: 0,
