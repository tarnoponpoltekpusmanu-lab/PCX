...
import { notifyVscodeFileUpdated } from 'src/brain_flowork_services/mcp/vscodeSdkMcp.js'
import type { LogOption } from 'src/brain_flowork_types/logs.js'
...
export function fileHistoryEnabled(): boolean {
  ...
  return (
    ...
    !isEnvTruthy(process.env.FLOWORK_ENGINE_DISABLE_FILE_CHECKPOINTING)
  )
}

function fileHistoryEnabledSdk(): boolean {
  return (
    isEnvTruthy(process.env.FLOWORK_ENGINE_ENABLE_SDK_FILE_CHECKPOINTING) &&
    !isEnvTruthy(process.env.FLOWORK_ENGINE_DISABLE_FILE_CHECKPOINTING)
  )
}
...
export async function fileHistoryMakeSnapshot(
  ...
  // for any wrapper that honors same-ref returns (src/FLOWORK.md wrapper
