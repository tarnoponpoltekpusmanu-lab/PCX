...
function createHttpsProxyAgent(
  ...
  if (isEnvTruthy(process.env.FLOWORK_ENGINE_PROXY_RESOLVES_HOSTS)) {
...
/**
 * Get fetch options for the FloworkOS SDK with proxy and mTLS configuration
 * Returns fetch options with appropriate dispatcher for proxy and/or mTLS
 *
 * @param opts.forFloworkOSAPI - Enables FLOWORKOS_UNIX_SOCKET tunneling. This
 ...
 *   into non-FloworkOS-API fetch paths (MCP HTTP/SSE transports, etc.) or those
 *   requests get misrouted to api.floworkos.com. Only the FloworkOS SDK client
 *   should pass `true` here.
 */
export function getProxyFetchOptions(opts?: { forFloworkOSAPI?: boolean }): {
  tls?: TLSConfig
  ...
  // FLOWORKOS_UNIX_SOCKET tunnels through the `claude ssh` auth proxy, which
  // hardcodes the upstream to the FloworkOS API. Scope to the FloworkOS API
  // client so MCP/SSE/other callers don't get their requests misrouted.
  if (opts?.forFloworkOSAPI) {
    const unixSocket = process.env.FLOWORKOS_UNIX_SOCKET
