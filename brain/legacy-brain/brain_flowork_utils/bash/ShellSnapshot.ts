...
/**
 * Generates Flowork AI Engine specific snapshot content
 * This content is always included regardless of user configuration
 */
async function getFloworkEngineSnapshotContent(): Promise<string> {
...
RIPGREP_FUNC_END
  ...
  // consistent fast behavior in Flowork's shell.
...
async function getSnapshotScript(
  ...
  // Generate the user content and Flowork AI Engine content
  const userContent = configFileExists
    ...
    : !isZsh
      ? // we need to manually force alias expansion in bash - normally `getUserSnapshotContent` takes care of this
        'echo "shopt -s expand_aliases" >> "$SNAPSHOT_FILE"'
      : ''
  const floworkEngineContent = await getFloworkEngineSnapshotContent()
...
export const createAndSaveSnapshot = async (
  ...
  return new Promise(async resolve => {
    try {
      ...
      if (!configFileExists) {
        logForDebugging(
          `Shell config file not found: ${configFile}, creating snapshot with Flowork AI Engine defaults only`,
        )
      }
      ...
      execFile(
        ...
        {
          env: {
            ...((process.env.FLOWORK_ENGINE_DONT_INHERIT_ENV
              ? {}
