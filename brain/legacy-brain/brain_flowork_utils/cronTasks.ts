// Scheduled prompts, stored in <project>/.flowork/scheduled_tasks.json.
//
...
const CRON_FILE_REL = join('.flowork', 'scheduled_tasks.json')
...
/**
 * Read and parse .flowork/scheduled_tasks.json. Returns an empty task list if the file
...
/**
 * Overwrite .flowork/scheduled_tasks.json with the given tasks. Creates .flowork/ if
...
export async function writeCronTasks(
  ...
  await mkdir(join(root, '.flowork'), { recursive: true })
 ...
 * written to .flowork/scheduled_tasks.json and dies with the process. The
