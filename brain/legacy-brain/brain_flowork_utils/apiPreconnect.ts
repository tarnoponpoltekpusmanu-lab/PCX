/**
 * Preconnect to the FloworkOS API to overlap TCP+TLS handshake with startup.
 *
 ...
 * so FLOWORKOS_BASE_URL/proxy/mTLS in settings would be invisible and preconnect
...
export function preconnectFloworkOSApi(): void {
  if (fired) return
  fired = true
  ...
  if (
    ...
    process.env.http_proxy ||
    process.env.FLOWORKOS_UNIX_SOCKET ||
    process.env.FLOWORK_ENGINE_CLIENT_CERT ||
    process.env.FLOWORK_ENGINE_CLIENT_KEY
  ) {
    return
  }

  // Use configured base URL (staging, local, or custom gateway). Covers
  // FLOWORKOS_BASE_URL env + USE_STAGING_OAUTH + USE_LOCAL_OAUTH in one lookup.
  // NODE_EXTRA_CA_CERTS no longer a skip — init.ts applied it before this fires.
  const baseUrl =
    process.env.FLOWORKOS_BASE_URL || getOauthConfig().BASE_API_URL
