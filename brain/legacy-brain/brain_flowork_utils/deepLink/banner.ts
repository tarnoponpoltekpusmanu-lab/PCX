 ...
 * notice which directory — and therefore which FLOWORK.md — was loaded.
 */
 ...
 *
 * Always shows the working directory so the user can see which FLOWORK.md
 ...
 * and whether its FLOWORK.md may be stale relative to upstream.
 */
export function buildDeepLinkBanner(info: DeepLinkBannerInfo): string {
  ...
  if (info.repo) {
    ...
    lines.push(
      `Resolved ${info.repo} from local clones · last fetched ${age}${stale ? ' — FLOWORK.md may be stale' : ''}`,
    )
  }
