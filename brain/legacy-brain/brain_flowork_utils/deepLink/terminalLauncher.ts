 ...
 * Terminal Launcher
 *
 * Detects the user's preferred terminal emulator and launches Flowork AI Engine
...
/**
 * Launch Flowork AI Engine in the detected terminal emulator.
 *
...
async function launchMacosTerminal(
  ...
  activate
    ...
    case 'Ghostty': {
      ...
      args.push('-e', claudePath, ...floworkArgs)
    ...
    case 'kitty': {
      ...
      args.push(claudePath, ...floworkArgs)
      const { code } = await execFileNoThrow('open', args, { useCwd: false })
      if (code === 0) return true
      break
    }
...
function buildShellCommand(
  ...
  return `${cdPrefix}${[claudePath, ...floworkArgs].map(shellQuote).join(' ')}`
}
