...
export const MACOS_BUNDLE_ID = 'com.floworkos.flowork-engine-url-handler'
const APP_NAME = 'Flowork AI Engine URL Handler'
const DESKTOP_FILE_NAME = 'flowork-engine-url-handler.desktop'
const MACOS_APP_NAME = 'Flowork AI Engine URL Handler.app'
 ...
 * already-installed (and signed) `flowork` binary. When macOS opens a
 * `claude-cli://` URL, it launches `flowork` through this app bundle.
...
<dict>
  ...
  <array>
    <dict>
      ...
      <string>Flowork AI Engine Deep Link</string>
...
Name=${APP_NAME}
Comment=Handle ${DEEP_LINK_PROTOCOL}:// deep links for Flowork AI Engine
 ...
 * native installer's stable symlink (~/.local/bin/flowork) which survives
...
async function resolveClaudePath(): Promise<string> {
  const binaryName = process.platform === 'win32' ? 'claude.exe' : 'flowork'
 ...
 * points at the expected `flowork` binary. Reads the registration artifact
 * directly (symlink target, .desktop Exec line, registry value) rather than
 * a cached flag in ~/.flowork.json, so:
...
export async function ensureDeepLinkProtocolRegistered(): Promise<void> {
  ...
  // ~/.flowork (per-machine, not synced) rather than ~/.flowork.json (can sync).
