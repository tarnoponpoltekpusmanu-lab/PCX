 ...
 * via shell expansion (e.g., ${FLOWORKOS_API_KEY}) in Bash tool commands.
 *
...
const GHA_SUBPROCESS_SCRUB = [
  // FloworkOS auth — claude re-reads these per-request, subprocesses don't need them
  'FLOWORKOS_API_KEY',
  ...
  // flowork-engine-action-specific duplicates — action JS consumes these during
  // prepare, before spawning claude. ALL_INPUTS contains floworkos_api_key as JSON.
  'ALL_INPUTS',
 ...
 * servers, shell hooks).
 *
 * Gated on FLOWORK_ENGINE_SUBPROCESS_ENV_SCRUB. flowork-engine-action sets this
...
export function subprocessEnv(): NodeJS.ProcessEnv {
  ...
  if (!isEnvTruthy(process.env.FLOWORK_ENGINE_SUBPROCESS_ENV_SCRUB)) {
  ...
  for (const k of GHA_SUBPROCESS_SCRUB) {
    ...
    // secrets like INPUT_FLOWORKOS_API_KEY. No-op for vars that aren't action inputs.
    delete env[`INPUT_${k}`]
  }
  return env
}
