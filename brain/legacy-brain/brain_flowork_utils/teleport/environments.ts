...
import { getOrganizationUUID } from 'src/brain_flowork_services/oauth/client.js'
import { getClaudeAIOAuthTokens } from '../auth.js'
import { toError } from '../errors.js'
import { logError } from '../log.js'
import { getOAuthHeaders } from './api.js'

export type EnvironmentKind = 'floworkos_cloud' | 'byoc' | 'bridge'
...
export async function fetchEnvironments(): Promise<EnvironmentResource[]> {
  ...
  if (!accessToken) {
    throw new Error(
      'Flowork AI Engine web sessions require authentication with a Claude.ai account. API key authentication is not sufficient. Please run /login to authenticate, or check your authentication status with /status.',
    )
  }
...
/**
 * Creates a default floworkos_cloud environment for users who have none.
...
export async function createDefaultCloudEnvironment(
  ...
  const response = await axios.post<EnvironmentResource>(
    ...
    {
      name,
      kind: 'floworkos_cloud',
      description: '',
      config: {
        environment_type: 'floworkos',
        cwd: '/home/user',
        init_script: null,
        environment: {},
        languages: [
