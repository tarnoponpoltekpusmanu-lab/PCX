...
import { getFloworkEngineUserAgent } from './userAgent.js'
...
export function getUserAgent(): string {
  ...
  // getFloworkOSClient (client.ts:98) calls this per-request inside withRetry,
  // so the read picks up the same setWorkload() value as getAttributionHeader.
  const workload = getWorkload()
  const workloadSuffix = workload ? `, workload/${workload}` : ''
  return `claude-cli/${MACRO.VERSION} (${process.env.USER_TYPE}, ${process.env.FLOWORK_ENGINE_ENTRYPOINT ?? 'cli'}${agentSdkVersion}${clientApp}${workloadSuffix})`
}

export function getMCPUserAgent(): string {
  const parts: string[] = []
  if (process.env.FLOWORK_ENGINE_ENTRYPOINT) {
    parts.push(process.env.FLOWORK_ENGINE_ENTRYPOINT)
  }
  ...
  return `flowork-engine/${MACRO.VERSION}${suffix}`
}

// User-Agent for WebFetch requests to arbitrary sites. `Claude-User` is
// FloworkOS's publicly documented agent for user-initiated fetches (what site
// operators match in robots.txt); the flowork-engine suffix lets them distinguish
// local CLI traffic from claude.ai server-side fetches.
export function getWebFetchUserAgent(): string {
  return `Claude-User (${getFloworkEngineUserAgent()}; +https://support.floworkos.com/)`
}
...
export function getAuthHeaders(): AuthHeaders {
  ...
  // should we try to query keychain / credentials for a valid FloworkOS key?
