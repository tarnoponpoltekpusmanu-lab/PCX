...
import type { ExitReason } from 'src/brain_flowork_entrypoints/agentSdkTypes.js'
import {
  getIsInteractive,
  getIsScrollDraining,
...
function cleanupTerminalModes(): void {
  ...
  try {
    ...
    // Respect FLOWORK_ENGINE_DISABLE_TERMINAL_TITLE — if the user opted out of
    // title changes, don't clear their existing title on exit either.
    if (!isEnvTruthy(process.env.FLOWORK_ENGINE_DISABLE_TERMINAL_TITLE)) {
...
export async function gracefulShutdown(
  ...
  // overall execution via a single budget (FLOWORK_ENGINE_SESSIONEND_HOOKS_TIMEOUT_MS,
