 ...
 * pipeline from user input to first token arrival. Enable by setting FLOWORK_ENGINE_PROFILE_QUERY=1
 *
 ...
 * - query_client_creation_start/end: Creating FloworkOS client
...
const ENABLED = isEnvTruthy(process.env.FLOWORK_ENGINE_PROFILE_QUERY)
...
function getQueryProfileReport(): string {
  if (!ENABLED) {
    return 'Query profiling not enabled (set FLOWORK_ENGINE_PROFILE_QUERY=1)'
  }
