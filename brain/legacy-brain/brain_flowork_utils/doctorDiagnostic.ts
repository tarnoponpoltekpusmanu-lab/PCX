...
async function getInstallationPath(): Promise<string> {
  ...
  if (isInBundledMode()) {
    ...
    try {
      const path = await which('flowork')
      if (path) {
        return path
      }
    } catch {
    ...
    try {
      await getFsImplementation().stat(join(homedir(), '.local/bin/flowork'))
      return join(homedir(), '.local/bin/flowork')
    } catch {
      // Not found
    }
    return 'native'
...
> {
  ...
  const localPath = join(homedir(), '.flowork', 'local')
  if (await localInstallationExists()) {
    installations.push({ type: 'npm-local', path: localPath })
  }

  // Check for global npm installation
  const packagesToCheck = ['@floworkos-ai/flowork-engine']
  if (MACRO.PACKAGE_URL && MACRO.PACKAGE_URL !== '@floworkos-ai/flowork-engine') {
  ...
  if (npmResult.code === 0 && npmResult.stdout) {
    ...
    // First check for active installations via bin/flowork
    // Linux / macOS have prefix/bin/flowork and prefix/lib/node_modules
    // Windows has prefix/flowork and prefix/node_modules
    const globalBinPath = isWindows
      ? join(npmPrefix, 'flowork')
      : join(npmPrefix, 'bin', 'flowork')
    ...
    if (globalBinExists) {
      ...
      // When npm is installed via Homebrew, both can exist at /opt/homebrew/bin/flowork
      ...
      // If no bin/flowork exists, check for orphaned packages (no bin/flowork symlink)
  ...
  const nativeBinPath = join(homedir(), '.local', 'bin', 'flowork')
  try {
  ...
  if (config.installMethod === 'native') {
    const nativeDataPath = join(homedir(), '.local', 'share', 'flowork')
    try {
...
async function detectConfigurationIssues(
  ...
  if (type === 'npm-local') {
    ...
    const whichResult = await which('flowork')
    ...
    if (!claudeInPath && !validAlias) {
      if (existingAlias) {
        ...
        warnings.push({
          ...
          fix: `Alias exists but points to invalid target: ${existingAlias}. Update alias: alias claude="~/.flowork/local/claude"`,
        })
      } else {
        // No alias exists and not in PATH
        warnings.push({
          issue: 'Local installation not accessible',
          fix: 'Create alias: alias claude="~/.flowork/local/claude"',
        })
      }
    }
  }
  ...
  if (installationType === 'native') {
    ...
    for (const install of npmInstalls) {
      if (install.type === 'npm-global') {
        let uninstallCmd = 'npm -g uninstall @floworkos-ai/flowork-engine'
        if (
          MACRO.PACKAGE_URL &&
          MACRO.PACKAGE_URL !== '@floworkos-ai/flowork-engine'
        ) {
