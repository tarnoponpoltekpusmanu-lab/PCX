...
import { clearSystemPromptSections } from '../brain_flowork_constants/systemPromptSections.js'
import { restoreCostStateForSession } from '../brain_flowork_cost_tracker.js'
...
export function restoreSessionStateFromLog(
  ...
  if (feature('CONTEXT_COLLAPSE')) {
    ...
    ;(
      require('../brain_flowork_services/contextCollapse/persist.js') as typeof import('../services/contextCollapse/persist.js')
    ).restoreFromEntries(
 ...
 * Used for computing initial state before render (per FLOWORK.md guidelines).
...
export async function processResumedConversation(
  ...
  // Compute initial state before render (per FLOWORK.md guidelines)
