...
export function getLocalISODate(): string {
  ...
  if (process.env.FLOWORK_ENGINE_OVERRIDE_DATE) {
    return process.env.FLOWORK_ENGINE_OVERRIDE_DATE
  }
...
export function getLocalMonthYear(): string {
  const date = process.env.FLOWORK_ENGINE_OVERRIDE_DATE
    ? new Date(process.env.FLOWORK_ENGINE_OVERRIDE_DATE)
    : new Date()
