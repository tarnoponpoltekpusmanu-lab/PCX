...
import type { HistoryEntry, PastedContent } from './brain_flowork_utils/config.js'
...
import { readLinesReverse } from './brain_flowork_utils/fsOperations.js'
import { lock } from './brain_flowork_utils/lockfile.js'
import {
  hashPastedText,
  retrievePastedText,
  storePastedText,
} from './brain_flowork_utils/pasteStore.js'
...
/**
 * Flowork AI Engine parses history for pasted content references to match back to
...
export function addToHistory(command: HistoryEntry | string): void {
  // Skip history when running in a tmux session spawned by Flowork AI Engine's Tungsten tool.
  // This prevents verification/test sessions from polluting the user's real command history.
  if (isEnvTruthy(process.env.FLOWORK_ENGINE_SKIP_PROMPT_HISTORY)) {
    return
  }
