...
import { decodeJwtExpiry } from '../../brain_flowork_bridge/jwtUtils.js'
...
import { createAxiosInstance } from '../../brain_flowork_utils/proxy.js'
import {
...
} from '../../brain_flowork_utils/sessionIngressAuth.js'
import type {
 ...
 * - Epoch management: reads worker_epoch from FLOWORK_ENGINE_WORKER_EPOCH env var
...
export class CCRClient {
   ...
   * (FLOWORK_ENGINE_SESSION_ACCESS_TOKEN env var). Callers managing multiple
  ...
  constructor(
    ...
    opts?: {
       ...
       * FLOWORK_ENGINE_SESSION_ACCESS_TOKEN (single-session callers — REPL,
   ...
   *    FLOWORK_ENGINE_WORKER_EPOCH (set by env-manager / bridge spawner)
  ...
  async initialize(epoch?: number): Promise<Record<string, unknown> | null> {
    ...
    if (epoch === undefined) {
      const rawEpoch = process.env.FLOWORK_ENGINE_WORKER_EPOCH
