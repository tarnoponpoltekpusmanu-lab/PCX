 ...
 * (sessions_elevated_auth_enforcement in FloworkOS Main); this CLI-side
 ...
 *
 * See floworkoss/floworkos#274559 (spec), #310375 (B1b tenant RPCs),
...
export async function enrollTrustedDevice(): Promise<void> {
  try {
    ...
    const { getClaudeAIOAuthTokens } =
      require('../brain_flowork_utils/auth.js') as typeof import('../utils/auth.js')
    ...
    try {
      response = await axios.post<{
        ...
        { display_name: `Flowork AI Engine on ${hostname()} · ${process.platform}` },
        {
          headers: {
