 ...
 * (oauth-authed, same store), write to ~/.flowork/uploads/{sessionId}/, and
 * return @path refs to prepend. Flowork's Read tool takes it from there.
 *
...
async function resolveOne(att: InboundAttachment): Promise<string | undefined> {
  ...
  try {
    ...
    // FLOWORK_ENGINE_CUSTOM_OAUTH_URL — keep it inside the try so a bad
