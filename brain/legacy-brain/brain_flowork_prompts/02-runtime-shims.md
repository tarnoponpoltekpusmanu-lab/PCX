...
You are working in `/workspaces/flowork-engine`. This is the Flowork AI Engine CLI source. It was built to run under **Bun's bundler** which provides two build-time features that don't exist at runtime:
...
- `MACRO.PACKAGE_URL` — npm package name (e.g., `"@floworkos-ai/flowork-engine"`)
...
const FEATURE_FLAGS: Record<string, boolean> = {
  PROACTIVE: envBool('FLOWORK_ENGINE_PROACTIVE', false),
  KAIROS: envBool('FLOWORK_ENGINE_KAIROS', false),
  BRIDGE_MODE: envBool('FLOWORK_ENGINE_BRIDGE_MODE', false),
  DAEMON: envBool('FLOWORK_ENGINE_DAEMON', false),
  VOICE_MODE: envBool('FLOWORK_ENGINE_VOICE_MODE', false),
  AGENT_TRIGGERS: envBool('FLOWORK_ENGINE_AGENT_TRIGGERS', false),
  MONITOR_TOOL: envBool('FLOWORK_ENGINE_MONITOR_TOOL', false),
  COORDINATOR_MODE: envBool('FLOWORK_ENGINE_COORDINATOR_MODE', false),
  ABLATION_BASELINE: false,  // always off for external builds
  DUMP_SYSTEM_PROMPT: envBool('FLOWORK_ENGINE_DUMP_SYSTEM_PROMPT', false),
  BG_SESSIONS: envBool('FLOWORK_ENGINE_BG_SESSIONS', false),
}
...
const MACRO_OBJ = {
  VERSION: version,
  PACKAGE_URL: '@floworkos-ai/flowork-engine',
  ISSUES_EXPLAINER: 'report issues at https://github.com/floworkoss/flowork-engine/issues',
}
