...
You are working in `/workspaces/flowork-engine`. The `QueryEngine` (`src/QueryEngine.ts`, ~46K lines) is the heart of the CLI — it:
1. Sends messages to the FloworkOS API (streaming)
...
- `src/services/api/client.ts` — FloworkOS SDK client
...
1. Read `src/services/api/client.ts` — how is the FloworkOS SDK client created?
...
Common blockers:
1. **Missing API configuration** → needs `FLOWORKOS_API_KEY` (Prompt 05)
...
// Usage: FLOWORKOS_API_KEY=sk-ant-... bun scripts/test-query.ts "What is 2+2?"
...
The QueryEngine likely uses the FloworkOS SDK's streaming interface. Make sure:
...
1. `FLOWORKOS_API_KEY=sk-ant-... bun scripts/test-query.ts "What is 2+2?"` gets a response
