...
You are working in `/workspaces/flowork-engine`. The CLI renders its UI using **React + Ink** — a framework that renders React components to the terminal (not a browser). This project includes a **custom fork of Ink** embedded directly in `src/ink/`.
...
- `src/components/` — Flowork AI Engine's ~140 custom components
...
// Minimal component
function Hello() {
  return <Text>Hello from Flowork AI Engine Ink UI!</Text>
}
...
1. `scripts/test-ink.tsx` renders "Hello from Flowork AI Engine Ink UI!" to the terminal
