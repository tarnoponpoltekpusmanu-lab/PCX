...
You are working in `/workspaces/flowork-engine/mcp-server/`. This is a separate sub-project that provides an MCP (Model Context Protocol) server for exploring the Flowork AI Engine source. It's a simpler, self-contained TypeScript project.
...
1. **Run the build and capture errors**:
   ```bash
   cd /workspaces/flowork-engine/mcp-server
   npm run build 2>&1
   ```
