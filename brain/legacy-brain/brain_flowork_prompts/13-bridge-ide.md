...
You are working in `/workspaces/flowork-engine`. The "Bridge" is the subsystem that connects Flowork AI Engine to IDE extensions (VS Code, JetBrains). It enables:
- Remote control of Flowork AI Engine from an IDE
...
Ensure that when `FLOWORK_ENGINE_BRIDGE_MODE=false` (or unset):
...
1. Set `FLOWORK_ENGINE_BRIDGE_MODE=true`
