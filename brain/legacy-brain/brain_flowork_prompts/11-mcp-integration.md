...
You are working in `/workspaces/flowork-engine`. The CLI has built-in MCP (Model Context Protocol) support:
- **MCP Client** — connects to external MCP servers (tools, resources)
- **MCP Server** — exposes Flowork AI Engine itself as an MCP server

MCP lets the CLI use tools provided by external servers and lets other clients use Flowork AI Engine as a tool provider.
...
1. Where does the config file live? (`~/.flowork/.mcp.json`? project root?)
...
{
  "mcpServers": {
    "flowork-engine-explorer": {
      "command": "node",
      "args": ["mcp-server/dist/index.js"],
      "env": {
        "FLOWORK_ENGINE_SRC_ROOT": "./src"
      }
    }
  }
}
