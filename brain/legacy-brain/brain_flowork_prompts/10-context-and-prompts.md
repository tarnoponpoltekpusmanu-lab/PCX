...
You are working in `/workspaces/flowork-engine`. The CLI constructs a detailed system prompt before each conversation. This prompt includes:
...
4. **Memory** — persistent `.flowork.md` files (from `src/memdir/`)
...
- `src/memdir/` — Memory directory system (reads `.flowork.md`, `FLOWORK.md` files)
...
1. How does it find `.flowork.md` / `FLOWORK.md` files?
...
Verify it works by:
1. Creating a test `FLOWORK.md` in the project root
...
4. Memory system reads `.flowork.md` from the project root
