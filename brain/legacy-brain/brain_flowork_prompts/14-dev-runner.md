...
You are working in `/workspaces/flowork-engine`. By now you should have:
...
1. Set `FLOWORKOS_API_KEY` in `.env`
...
5. Run `FLOWORKOS_API_KEY=sk-ant-... bun run dev -p "say hello"` → should make one API call and print response
...
{
  "scripts": {
    "dev:debug": "FLOWORK_ENGINE_DEBUG_LOG_LEVEL=debug bun scripts/dev.ts"
  }
}
```
