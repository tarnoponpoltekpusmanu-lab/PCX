...
import type { LoadedPlugin } from '../../brain_flowork_types/plugin.js';
import { count } from '../../brain_flowork_utils/array.js';
import { shouldSkipPluginAutoupdate } from '../../brain_flowork_utils/config.js';
...
import { createPluginId, formatMarketplaceLoadingErrors, getMarketplaceSourceDisplay, loadMarketplacesWithGracefulDegradation } from '../../brain_flowork_utils/plugins/marketplaceHelpers.js';
import { loadKnownMarketplacesConfig, refreshMarketplace, removeMarketplaceSource, setMarketplaceAutoUpdate } from '../../brain_flowork_utils/plugins/marketplaceManager.js';
import { updatePluginsForMarketplaces } from '../../brain_flowork_utils/plugins/pluginAutoupdate.js';
import { loadAllPlugins } from '../../brain_flowork_utils/plugins/pluginLoader.js';
import { isMarketplaceAutoUpdate } from '../../brain_flowork_utils/plugins/schemas.js';
import { getSettingsForSource, updateSettingsForSource } from '../../brain_flowork_utils/settings/settings.js';
...
export function ManageMarketplaces({
  ...
  if (internalView === 'details' && selectedMarketplace) {
    ...
    return <Box flexDirection="column">
        ...
        {!isUpdating && !shouldSkipPluginAutoupdate() && selectedMarketplace.autoUpdate && <Box marginTop={1}>
              <Text dimColor>
                Auto-update enabled. Flowork AI Engine will automatically update this
  ...
  return <Box flexDirection="column">
              ...
              <Box flexDirection="column" flexGrow={1}>
                <Box flexDirection="row" gap={1}>
                  <Text bold strikethrough={state.pendingRemove} dimColor={state.pendingRemove}>
                    {state.name === 'claude-plugins-official' && <Text color="flowork">✻ </Text>}
                    {state.name}
                    {state.name === 'claude-plugins-official' && <Text color="flowork"> ✻</Text>}
                  </Text>
      ...
      {isProcessing && <Box marginTop={1}>
          <Text color="flowork">Processing changes…</Text>
        </Box>}
