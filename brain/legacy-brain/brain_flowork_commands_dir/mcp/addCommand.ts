...
import { cliError, cliOk } from '../../brain_flowork_cli/exit.js'
import {
...
import {
  readClientSecret,
  saveMcpClientSecret,
} from '../../brain_flowork_services/mcp/auth.js'
import { addMcpConfig } from '../../brain_flowork_services/mcp/config.js'
import {
  describeMcpConfigFilePath,
  ensureConfigScope,
  ensureTransport,
  parseHeaders,
} from '../../brain_flowork_services/mcp/utils.js'
import {
  getXaaIdpSettings,
  isXaaEnabled,
} from '../../brain_flowork_services/mcp/xaaIdpLogin.js'
import { parseEnvVars } from '../../brain_flowork_utils/envUtils.js'
...
export function registerMcpAddCommand(mcp: Command): void {
  mcp
    ...
    .description(
      'Add an MCP server to Flowork AI Engine.\n\n' +
        'Examples:\n' +
        '  # Add HTTP server:\n' +
    ...
    .action(async (name, commandOrUrl, args, options) => {
      ...
      try {
        ...
        if (options.xaa && !isXaaEnabled()) {
          cliError(
            'Error: --xaa requires FLOWORK_ENGINE_ENABLE_XAA=1 in your environment',
          )
        }
