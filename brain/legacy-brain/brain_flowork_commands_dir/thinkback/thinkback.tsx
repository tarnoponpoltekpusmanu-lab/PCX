...
import type { CommandResultDisplay } from '../../brain_flowork_commands.js';
import { Select } from '../../components/CustomSelect/select.js';
import { Dialog } from '../../components/design-system/Dialog.js';
import { Spinner } from '../../components/Spinner.js';
import instances from '../../ink/instances.js';
import { Box, Text } from '../../ink.js';
import { enablePluginOp } from '../../brain_flowork_services/plugins/pluginOperations.js';
import { logForDebugging } from '../../brain_flowork_utils/debug.js';
import { isENOENT, toError } from '../../brain_flowork_utils/errors.js';
import { execFileNoThrow } from '../../brain_flowork_utils/execFileNoThrow.js';
import { pathExists } from '../../brain_flowork_utils/file.js';
...
import { isPluginInstalled } from '../../brain_flowork_utils/plugins/installedPluginsManager.js';
import { addMarketplaceSource, clearMarketplacesCache, loadKnownMarketplacesConfig, refreshMarketplace } from '../../brain_flowork_utils/plugins/marketplaceManager.js';
import { OFFICIAL_MARKETPLACE_NAME } from '../../brain_flowork_utils/plugins/officialMarketplace.js';
import { loadAllPlugins } from '../../brain_flowork_utils/plugins/pluginLoader.js';
import { installSelectedPlugins } from '../../brain_flowork_utils/plugins/pluginStartupCheck.js';

// Marketplace and plugin identifiers - varies by user type
const INTERNAL_MARKETPLACE_NAME = 'flowork-engine-marketplace';
const INTERNAL_MARKETPLACE_REPO = 'floworkoss/flowork-engine-marketplace';
const OFFICIAL_MARKETPLACE_REPO = 'floworkoss/claude-plugins-official';
...
export async function playAnimation(skillDir: string): Promise<{
  ...
  // silently return success. Using readFile (not access) per FLOWORK.md.
  //
...
function ThinkbackMenu(t0) {
  ...
  let t4;
  if ($[8] !== hasGenerated) {
    t4 = !hasGenerated && <Box flexDirection="column"><Text>Relive your year of coding with Flowork AI.</Text><Text dimColor={true}>{"We'll create a personalized ASCII animation celebrating your journey."}</Text></Box>;
    $[8] = hasGenerated;
    $[9] = t4;
  } else {
    t4 = $[9];
  ...
  if ($[16] !== handleCancel || $[17] !== t6) {
    t7 = <Dialog title="Think Back on 2025 with Flowork AI Engine" subtitle="Generate your 2025 Flowork AI Engine Think Back (takes a few minutes to run)" onCancel={handleCancel} color="flowork">{t6}</Dialog>;
    $[16] = handleCancel;
    $[17] = t6;
    $[18] = t7;
  } else {
...
const EDIT_PROMPT = 'Use the Skill tool to invoke the "thinkback" skill with mode=edit to modify my existing Flowork AI Engine year in review animation. Ask me what I want to change. When the animation is ready, tell the user to run /think-back again to play it.';
const FIX_PROMPT = 'Use the Skill tool to invoke the "thinkback" skill with mode=fix to fix validation or rendering errors in my existing Flowork AI Engine year in review animation. Run the validator, identify errors, and fix them. When the animation is ready, tell the user to run /think-back again to play it.';
const REGENERATE_PROMPT = 'Use the Skill tool to invoke the "thinkback" skill with mode=regenerate to create a completely new Flowork AI Engine year in review animation from scratch. Delete the existing animation and start fresh. When the animation is ready, tell the user to run /think-back again to play it.';
