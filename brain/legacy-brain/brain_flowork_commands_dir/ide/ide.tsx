...
import type { CommandResultDisplay, LocalJSXCommandContext } from '../../brain_flowork_commands.js';
...
import { clearServerCache } from '../../brain_flowork_services/mcp/client.js';
import type { ScopedMcpServerConfig } from '../../brain_flowork_services/mcp/types.js';
import { useAppState, useSetAppState } from '../../brain_flowork_state/AppState.js';
import { getCwd } from '../../brain_flowork_utils/cwd.js';
import { execFileNoThrow } from '../../brain_flowork_utils/execFileNoThrow.js';
import { type DetectedIDEInfo, detectIDEs, detectRunningIDEs, type IdeType, isJetBrainsIde, isSupportedJetBrainsTerminal, isSupportedTerminal, toIDEDisplayName } from '../../brain_flowork_utils/ide.js';
...
function IDEScreen(t0) {
  ...
  if ($[17] !== availableIDEs.length) {
    t5 = availableIDEs.length === 0 && <Text dimColor={true}>{isSupportedJetBrainsTerminal() ? "No available IDEs detected. Please install the plugin and restart your IDE:\nhttps://docs.flowork.com/s/flowork-engine-jetbrains" : "No available IDEs detected. Make sure your IDE has the Flowork AI Engine extension or plugin installed and is running."}</Text>;
  ...
  let t7;
  if ($[24] !== availableIDEs) {
    t7 = availableIDEs.length !== 0 && availableIDEs.some(_temp2) && <Box marginTop={1}><Text color="warning">Note: Only one Flowork AI Engine instance can be connected to VS Code at a time.</Text></Box>;
    $[24] = availableIDEs;
    $[25] = t7;
  } else {
    t7 = $[25];
...
export async function call(onDone: (result?: string, options?: {
  ...
  if (args?.trim() === 'open') {
    ...
    if (availableIDEs.length === 0) {
      onDone('No IDEs with Flowork AI Engine extension detected.');
      return null;
    }
