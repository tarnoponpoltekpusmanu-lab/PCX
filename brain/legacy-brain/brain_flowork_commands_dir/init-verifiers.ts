...
3. **Installed verification packages** (for web apps)
   ...
   - Check MCP configuration (.mcp.json) for browser automation tools:
     ...
     - Claude Chrome Extension MCP (browser-use via Flowork's Chrome extension)
...
**All verifier skills are created in the project root's \`.flowork/skills/\` directory.** This ensures they are automatically loaded when Claude runs in the project.

Write the skill file to \`.flowork/skills/<verifier-name>/SKILL.md\`.
...
1. Where each skill was created (always in \`.flowork/skills/\`)
