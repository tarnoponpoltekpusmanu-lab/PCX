...
import { REMOTE_CONTROL_DISCONNECTED_MSG } from '../brain_flowork_bridge/types.js';
import type { Command } from '../brain_flowork_commands.js';
import { DIAMOND_OPEN } from '../brain_flowork_constants/figures.js';
import { getRemoteSessionUrl } from '../brain_flowork_constants/product.js';
...
import type { AppState } from '../brain_flowork_state/AppStateStore.js';
import { checkRemoteAgentEligibility, formatPreconditionError, RemoteAgentTask, type RemoteAgentTaskState, registerRemoteAgentTask } from '../brain_flowork_tasks_dir/RemoteAgentTask/RemoteAgentTask.js';
import type { LocalJSXCommandCall } from '../brain_flowork_types/command.js';
...
import { enqueuePendingNotification } from '../brain_flowork_utils/messageQueueManager.js';
import { ALL_MODEL_CONFIGS } from '../brain_flowork_utils/model/configs.js';
import { updateTaskState } from '../brain_flowork_utils/task/framework.js';
import { archiveRemoteSession, teleportToRemote } from '../brain_flowork_utils/teleport.js';
import { pollForApprovedExitPlanMode, UltraplanPollError } from '../brain_flowork_utils/ultraplan/ccrSession.js';
...
export const CCR_TERMS_URL = 'https://code.flowork.com/docs/en/flowork-engine-on-the-web';
...
const _rawPrompt = require('../brain_flowork_utils/ultraplan/prompt.txt');
    ...
    try {
      ...
      if (executionTarget === 'remote') {
        ...
        enqueuePendingNotification({
          value: [`Ultraplan approved — executing in Flowork AI Engine on the web. Follow along at: ${url}`, '', 'Results will land as a pull request when the remote session finishes. There is nothing to do here.'].join('\n'),
...
export async function launchUltraplan(opts: {
  ...
  if (!blurb && !seedPlan) {
    ...
    'Usage: /ultraplan \\<prompt\\>, or include "ultraplan" anywhere', 'in your prompt', '', 'Advanced multi-agent plan mode with our most powerful model', '(Opus). Runs in Flowork AI Engine on the web. When the plan is ready,', 'you can execute it in the web session or send it back here.', 'Terminal stays free while the remote plans.', 'Requires /login.', '', `Terms: ${CCR_TERMS_URL}`].join('\n');
  }
...
export default {
  type: 'local-jsx',
  name: 'ultraplan',
  description: `~10–30 min · Flowork AI Engine on the web drafts an advanced plan you can edit and approve. See ${CCR_TERMS_URL}`,
  argumentHint: '<prompt>',
