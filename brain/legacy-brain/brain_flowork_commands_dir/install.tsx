...
import { checkInstall, cleanupNpmInstallations, cleanupShellAliases, installLatest } from '../brain_flowork_utils/nativeInstaller/index.js';
import { getInitialSettings, updateSettingsForSource } from '../brain_flowork_utils/settings/settings.js';
...
function Install({
  ...
  useEffect(() => {
    if (state.type === 'success') {
      ...
      setTimeout(onDone, 2000, 'Flowork AI Engine installation completed successfully', {
        display: 'system' as const
      });
    } else if (state.type === 'error') {
      // Give error message time to render before exiting
      setTimeout(onDone, 3000, 'Flowork AI Engine installation failed', {
        display: 'system' as const
      });
    }
  }, [state, onDone]);
  return <Box flexDirection="column" marginTop={1}>
      {state.type === 'checking' && <Text color="flowork">Checking installation status...</Text>}

      {state.type === 'cleaning-npm' && <Text color="warning">Cleaning up old npm installations...</Text>}

      {state.type === 'installing' && <Text color="flowork">
          Installing Flowork AI Engine native build {state.version}...
        </Text>}

      {state.type === 'setting-up' && <Text color="flowork">Setting up launcher and shell integration...</Text>}
      ...
      {state.type === 'success' && <Box flexDirection="column" gap={1}>
          <Box>
            ...
            <Text color="success" bold>
              Flowork AI Engine successfully installed!
            </Text>
          </Box>
          <Box marginLeft={2} flexDirection="column" gap={1}>
            {state.version !== 'current' && <Box>
                <Text dimColor>Version: </Text>
                <Text color="flowork">{state.version}</Text>
              </Box>}
            <Box>
...
export const install = {
  ...
  name: 'install',
  description: 'Install Flowork AI Engine native build',
