 ...
 *   1. FLOWORK_ENGINE_DISABLE_AUTO_MEMORY env var (1/true → OFF, 0/false → ON)
 *   2. FLOWORK_ENGINE_SIMPLE (--bare) → OFF
 *   3. CCR without persistent storage → OFF (no FLOWORK_ENGINE_REMOTE_MEMORY_DIR)
 *   4. autoMemoryEnabled in settings.json (supports project-level opt-out)
 *   5. Default: enabled
 */
export function isAutoMemoryEnabled(): boolean {
  const envVal = process.env.FLOWORK_ENGINE_DISABLE_AUTO_MEMORY
 ...
 * Resolution order:
 *   1. FLOWORK_ENGINE_REMOTE_MEMORY_DIR env var (explicit override, set in CCR)
 *   2. ~/.flowork (default config home)
 */
export function getMemoryBaseDir(): string {
  if (process.env.FLOWORK_ENGINE_REMOTE_MEMORY_DIR) {
    return process.env.FLOWORK_ENGINE_REMOTE_MEMORY_DIR
  }
 ...
 *
 * SECURITY: projectSettings (.flowork/settings.json committed to the repo) is
 ...
 * same repo share one auto-memory directory (floworkoss/flowork-engine#24382).
 */
