...
import { REMOTE_TRIGGER_TOOL_NAME } from '../../brain_flowork_tools/RemoteTriggerTool/prompt.js'
import { getClaudeAIOAuthTokens } from '../../brain_flowork_utils/auth.js'
import { checkRepoForRemoteAccess } from '../../brain_flowork_utils/background/remote/preconditions.js'
import { logForDebugging } from '../../brain_flowork_utils/debug.js'
import {
  detectCurrentRepositoryWithHost,
  parseGitRemote,
} from '../../brain_flowork_utils/detectRepository.js'
import { getRemoteUrl } from '../../brain_flowork_utils/git.js'
import { jsonStringify } from '../../brain_flowork_utils/slowOperations.js'
import {
  createDefaultCloudEnvironment,
  type EnvironmentResource,
  fetchEnvironments,
} from '../../brain_flowork_utils/teleport/environments.js'
...
You are helping the user schedule, update, list, or run **remote** Flowork AI Engine agents. These are NOT local cron jobs — each trigger spawns a fully isolated remote session (CCR) in FloworkOS's cloud infrastructure on a cron schedule. The agent runs in a sandboxed environment with its own git checkout, tools, and optional MCP connections.
...
- These are REMOTE agents — they run in FloworkOS's cloud, not on the user's machine. They cannot access local files, local services, or local environment variables.
...
export function registerScheduleRemoteAgentsSkill(): void {
  registerBundledSkill({
    ...
    whenToUse:
      'When the user wants to schedule a recurring remote agent, set up automated tasks, create a cron job for Flowork AI Engine, or manage their scheduled agents/triggers.',
    userInvocable: true,
    isEnabled: () =>
