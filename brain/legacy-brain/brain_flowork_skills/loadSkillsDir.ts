...
import type { Command, PromptCommand } from '../brain_flowork_types/command.js'
import {
  parseArgumentNames,
  substituteArguments,
} from '../brain_flowork_utils/argumentSubstitution.js'
...
import { isENOENT, isFsInaccessible } from '../brain_flowork_utils/errors.js'
import {
...
} from '../brain_flowork_utils/frontmatterParser.js'
import { getFsImplementation } from '../brain_flowork_utils/fsOperations.js'
import { isPathGitignored } from '../brain_flowork_utils/git/gitignore.js'
...
} from '../brain_flowork_utils/markdownConfigLoader.js'
import { parseUserSpecifiedModel } from '../brain_flowork_utils/model/model.js'
import { executeShellCommandsInPrompt } from '../brain_flowork_utils/promptShellExecution.js'
import type { SettingSource } from '../brain_flowork_utils/settings/constants.js'
...
import { HooksSchema, type HooksSettings } from '../brain_flowork_utils/settings/types.js'
import { createSignal } from '../brain_flowork_utils/signal.js'
...
export function getSkillsPath(
  ...
  switch (source) {
    case 'policySettings':
      return join(getManagedFilePath(), '.flowork', dir)
    case 'userSettings':
 ...
 * See: https://github.com/floworkoss/flowork-engine/issues/13893
 */
...
/**
 * Parse paths frontmatter from a skill, using the same format as FLOWORK.md rules.
...
export const getSkillDirCommands = memoize(
  async (cwd: string): Promise<Command[]> => {
    ...
    const managedSkillsDir = join(getManagedFilePath(), '.flowork', 'skills')
    ...
    const [
      ...
      isEnvTruthy(process.env.FLOWORK_ENGINE_DISABLE_POLICY_SKILLS)
        ? Promise.resolve([])
...
export async function discoverSkillDirsForPaths(
  ...
  for (const filePath of filePaths) {
    ...
    while (currentDir.startsWith(resolvedCwd + pathSep)) {
      const skillDir = join(currentDir, '.flowork', 'skills')
      ...
      if (!dynamicSkillDirs.has(skillDir)) {
        ...
        try {
          ...
          // is gitignored — blocks e.g. node_modules/pkg/.flowork/skills from
 ...
 * of FLOWORK.md conditional rules.
 *
