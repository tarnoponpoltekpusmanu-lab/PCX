/**
 * Claude service — wraps the FloworkOS SDK for streaming chat.
 *
 * The service:
 *   1. Builds the message history from the DB
 *   2. Opens a streaming connection to the FloworkOS API
...
import type { MessageParam, Tool } from "@floworkos-ai/sdk/resources/index.js";
...
export async function streamMessage(opts: StreamMessageOptions): Promise<void> {
  ...
  const client = new FloworkOS({
    apiKey: apiKey ?? process.env.FLOWORKOS_API_KEY,
  });
  ...
  try {
    ...
    while (continueLoop) {
      ...
      const toolUseBlocks = snapshot.content.filter(
        (b): b is FloworkOS.ToolUseBlock => b.type === "tool_use",
      );

      if (toolUseBlocks.length === 0) break; // No tool calls → done

      continueLoop = true;
      const toolResults: FloworkOS.ToolResultBlockParam[] = [];
