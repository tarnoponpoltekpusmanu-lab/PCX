 ...
 *   - API key pass-through (AUTH_PROVIDER=apikey, key forwarded to FloworkOS)
 *
...
export interface ApiUser {
  id: string;
  isAdmin: boolean;
  apiKey?: string; // FloworkOS API key (from header or env)
}
...
export function requireAuth(req: Request, res: Response, next: NextFunction): void {
  ...
  if (AUTH_PROVIDER === "apikey") {
    ...
    const apiKey = token ?? process.env.FLOWORKOS_API_KEY;
    if (!apiKey) {
      res.status(401).json({
        error: { code: "UNAUTHORIZED", message: "FloworkOS API key required" },
      });
      return;
    }
