...
/** Register an FloworkOS API reachability check. */
export function registerFloworkOSCheck(): void {
  registerCheck("floworkos_api", async () => {
    const start = Date.now();
    try {
      const controller = new AbortController();
      const timeout = setTimeout(() => controller.abort(), 4_000);
      const resp = await fetch("https://api.floworkos.com/v1/models", {
        method: "GET",
        headers: { "floworkos-version": "2023-06-01" },
