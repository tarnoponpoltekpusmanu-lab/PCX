...
const CLAUDE_BIN = process.env.CLAUDE_BIN ?? "flowork";
...
function userHomeDir(userId: string): string {
  ...
  try {
    mkdirSync(path.join(dir, ".flowork"), { recursive: true });
  } catch {
  ...
  (cols, rows, user?: AuthUser) => {
    ...
    return spawn(CLAUDE_BIN, [], {
      ...
      env: {
        ...
        ...(user?.apiKey ? { FLOWORKOS_API_KEY: user.apiKey } : {}),
      },
    });
  },
  GRACE_PERIOD_MS,
...
// Register the FloworkOS API reachability check.
registerFloworkOSCheck();
