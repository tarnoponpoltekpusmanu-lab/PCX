 ...
 *
 * Each user provides their own FloworkOS API key on the login page.
 * The key is stored encrypted in the server-side session and is injected
 * as `FLOWORKOS_API_KEY` into every PTY spawned for that user.
...
<head>
  ...
  <title>Flowork AI Engine — Sign In</title>
  <style>
...
<body>
  <div class="card">
    <h1>Flowork AI Engine</h1>
    <p class="subtitle">Enter your FloworkOS API key to start a session.</p>
    <!--ERROR-->
    <form method="POST" action="/auth/login">
      <label for="api_key">FloworkOS API Key</label>
      <input
        type="password"
        id="api_key"
        name="api_key"
    ...
    <p class="hint">
      ...
      Get a key at <a href="https://console.floworkos.com" target="_blank" rel="noopener">console.floworkos.com</a>.
    </p>
  </div>
</body>
</html>`;
