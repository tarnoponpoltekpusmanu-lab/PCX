...
export const ThinkingConfigSchema = lazySchema(() =>
  z
    ...
    .describe(
      "Controls Flowork's thinking/reasoning behavior. When set, takes precedence over the deprecated maxThinkingTokens.",
    ),
)
  ...
  z
    .object({
      ...
      apiProvider: z
        ...
        .optional()
        .describe(
          'Active API backend. FloworkOS OAuth login only applies when "firstParty"; for 3P providers the other fields are absent and auth is external (AWS creds, gcloud ADC, etc.).',
        ),
    })
  ...
  z
    .object({
      ...
      memory: z
        ...
        .optional()
        .describe(
          "Scope for auto-loading agent memory files. 'user' - ~/.flowork/agent-memory/<agentType>/, 'project' - .flowork/agent-memory/<agentType>/, 'local' - .flowork/agent-memory-local/<agentType>/",
        ),
      effort: z
  ...
  z
    ...
    .describe(
      'Source for loading filesystem-based settings. ' +
        "'user' - Global user settings (~/.flowork/settings.json). " +
        "'project' - Project settings (.flowork/settings.json). " +
        "'local' - Local settings (.flowork/settings.local.json).",
    ),
)
...
/** Placeholder for APIUserMessage from @floworkos-ai/sdk */
export const APIUserMessagePlaceholder = lazySchema(() => z.unknown())

/** Placeholder for APIAssistantMessage from @floworkos-ai/sdk */
export const APIAssistantMessagePlaceholder = lazySchema(() => z.unknown())

/** Placeholder for RawMessageStreamEvent from @floworkos-ai/sdk */
