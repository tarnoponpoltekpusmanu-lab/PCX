...
import type { FileHistorySnapshot } from 'src/brain_flowork_utils/fileHistory.js'
import type { ContentReplacementRecord } from 'src/brain_flowork_utils/toolResultStorage.js'
...
export type SerializedMessage = Message & {
  cwd: string
  userType: string
  entrypoint?: string // FLOWORK_ENGINE_ENTRYPOINT — distinguishes cli/sdk-ts/sdk-py/etc.
  sessionId: string
  timestamp: string
  version: string
  gitBranch?: string
...
/**
 * Per-file attribution state tracking Flowork's character contributions.
 */
export type FileAttributionState = {
  contentHash: string // SHA-256 hash of file content
  claudeContribution: number // Characters written by Flowork AI
  mtime: number // File modification time
}

/**
 * Attribution snapshot message stored in session transcript.
 * Tracks character-level contributions by Flowork AI for commit attribution.
 */
